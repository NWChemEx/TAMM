...
RUN export INSTALL_PATH=`pwd`/install; cmake-3.16.3-Linux-x86_64/bin/cmake -H. -Bbuild -DBUILD_TESTS=ON -DCATCH_ENABLE_COVERAGE=ON -DMPIEXEC_NUMPROC_FLAG="--allow-run-as-root -n" -DMPIEXEC_POSTFLAGS="--allow-run-as-root" -DCMAKE_CXX_FLAGS="-O0 --coverage" -DCMAKE_C_FLAGS="-O0 --coverage" -DCMAKE_Fortran_FLAGS="-O0 --coverage" -DCMAKE_EXE_LINKER_FLAGS="-O0 -fprofile-arcs" -DCMAKE_PREFIX_PATH=${INSTALL_PATH} -DCMAKE_BUILD_TYPE=DEBUG
...
RUN export LIBINT_DATA_PATH=/TAMM/TAMM/build/stage/usr/local/share/libint/2.6.0/basis; export OMPI_ALLOW_RUN_AS_ROOT=1; export OMPI_ALLOW_RUN_AS_ROOT_CONFIRM=1; cd build; make VERBOSE=1; ../cmake-3.16.3-Linux-x86_64/bin/ctest -VV
