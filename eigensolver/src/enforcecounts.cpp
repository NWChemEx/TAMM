#include "enforcecounts.hpp"
#include <algorithm>
#ifdef USE_MPI
#include <mpi.h>
#endif

using Eigen::MatrixXd;
using Eigen::VectorXd;
using Eigen::VectorXi;
using std::cout;
using std::endl;
using std::min;
using std::scientific;
using std::showpos;
using std::noshowpos;

#ifdef USE_MPI
void enforcecounts(MPI_Comm comm, slice *slices, int nslices, int n, int nev, VectorXi *inds) {
#else
void enforcecounts(slice *slices, int nslices, int n, int nev, VectorXi *inds) {
#endif
   int inrtcount, accboth, accslice, accleft, acctotal;
   double midpt;

   VectorXd sliceevals, sliceresnrms, tempresnrms;
   VectorXi lengths = VectorXi::Zero(nslices);
   VectorXi originds, tempvec;

   for (int i = 0; i < nslices; i++) {
      inds[i].resize(nev);
   }

   cout << "enforcing first slice" << endl;
   // first slice
   inrtcount = slices[0].nleft; // inertia count for the interval (-inf,shifts(0))
   accboth = slices[0].evals.size(); // number of eigenvalues within the target slices[0]
   accslice = 0; acctotal = 0;

   sliceevals.resize(accboth);
   sliceevals.setZero();
   sliceresnrms.resize(accboth);
   sliceresnrms.setZero();

   originds.resize(accboth);

   // pick out the approximate eigenvalues to the left of the shift
   // from the target slices[0]; save the corresponding residuals
   cout << "  pooling eigenvalues" << endl;
   for (int j = 0; j < slices[0].evals.size(); j++) {
      if (slices[0].evals(j) < slices[0].shift) {
         sliceevals(accslice) = slices[0].evals(j);
         sliceresnrms(accslice) = slices[0].resnrms(j);
         originds(accslice) = j;
         accslice++;
      }
   }

   std::cout << "### slice: " << 0 << ", right shift: " << slices[0].shift << ", num in slice: " << accslice << ", inertial count: " << inrtcount << std::endl;

   // sort the residuals associated with the selected eigenvalues
   VectorXi resinds = sortinds(sliceresnrms.head(accslice));

   cout << scientific;

   // copy the original indices of the selected eigenvalues in 
   // the target slices[0]
   for (int j = 0; j < min(inrtcount,accslice); j++) {
      inds[0](lengths(0)) = originds(resinds[j]);
      cout << "n: " << acctotal << ", slice: " << 0 << ", ind: " << originds(resinds[j]) << ", eval: " << sliceevals[resinds[j]] << ", resnrm: " << sliceresnrms[resinds[j]] << endl;
      acctotal++;
      lengths(0)++;
   }

   // all other slices
   for (int i = 1; i < nslices; i++) {
      // expected number of eigenvalues between shifts(i-1) and shifts(i) (inertial count
      inrtcount = slices[i].nleft - slices[i-1].nleft;
      // the total number of eigenvalues from targest slices i-1 and i
      accboth = slices[i-1].evals.size() + slices[i].evals.size();
      accslice = 0; accleft = 0;
      midpt = (slices[i].shift + slices[i-1].shift)/2.0;

      sliceevals.resize(accboth);
      sliceresnrms.resize(accboth);

      originds.resize(accboth);

      // select eigenvalues within the interval [shift(i-1),shifts(i)]
      // from the target slice i-1
      for (int j = 0; j < slices[i-1].evals.size(); j++) {
         if (slices[i-1].evals(j) >= slices[i-1].shift && slices[i-1].evals(j) < midpt) {
            sliceevals(accslice) = slices[i-1].evals(j);
            sliceresnrms(accslice) = slices[i-1].resnrms(j);
            originds(accslice) = j;
            accslice++;
            accleft++;
         }
      }
      // select eigenvalues within the interval [shift(i-1),shifts(i)]
      // from the target slice i
      for (int j = 0; j < slices[i].evals.size(); j++) {
         if (slices[i].evals(j) >= midpt && slices[i].evals(j) < slices[i].shift) {
            sliceevals(accslice) = slices[i].evals(j);
            sliceresnrms(accslice) = slices[i].resnrms(j);
            originds(accslice) = j;
            accslice++;
         }
      }

      std::cout << "### slice: " << i << ", right shift: " << slices[i].shift << ", num in slice: " << accslice << ", inertial count: " << inrtcount << std::endl;

      // sort the residual norms associated with the selected 
      // eigenvalues and return the sorted indices
      VectorXi resinds = sortinds(sliceresnrms.head(accslice));

      // copy the eigenvectors associated with the converged eigenvalues 
      // back to target slices to prepare the starting guess for 
      // the next SCF iteration
      if (acctotal + min(accslice,inrtcount) <= nev) { 
         cout << scientific; 
         for (int j = 0; j < min(accslice,inrtcount); j++) {
            if (resinds[j] < accleft) {
               inds[i-1](lengths(i-1)) = originds(resinds[j]);
               lengths(i-1)++;
               cout << "n: " << acctotal << ", slice: " << i-1 << ", ind: " << originds(resinds[j]) << ", eval: " << sliceevals(resinds[j]) << ", resnrm: " << sliceresnrms[resinds[j]] << endl;
            }
            else {
               inds[i](lengths(i)) = originds(resinds[j]);  
               lengths(i)++;
               cout << "n: " << acctotal << ", slice: " << i << ", ind: " << originds(resinds[j]) << ", eval: " << sliceevals[resinds[j]] << ", resnrm: " << sliceresnrms[resinds[j]] << endl;
            }
            acctotal++;
         }
      }
      else {
         /*for (int j = 0; j < accslice; j++) {
            cout << "--- eval: " << sliceevals[j] << ", resnrm: " << sliceresnrms[j] << endl;
         }*/
         // look at the remaining eigenvalues and pick ones with
         // small residual norms?
         VectorXi evalinds = sortinds(sliceevals.head(accslice));
         /*cout << "resinds: " << endl;
         cout << resinds << endl;
         cout << "evalinds: " << endl;
         cout << evalinds << endl;*/
         int j = 0; 
         cout << scientific; 
         bool accept;
         while (acctotal < nev && j < accslice) {
            accept = false;
            for (int l = 0; l < min(accslice,inrtcount); l++){
               if (evalinds[j] == resinds[l]) {
                  accept = true;
               }
            }
            if (accept) {
               if (evalinds[j] < accleft) {
                  inds[i-1](lengths(i-1)) = originds(evalinds[j]);
                  lengths(i-1)++;
                  cout << "n: " << acctotal << ", slice: " << i-1 << ", ind: " << originds(evalinds[j]) << ", eval: " << sliceevals[evalinds[j]] << ", resnrm: " << sliceresnrms[evalinds[j]] << endl;
               }
               else {
                  inds[i](lengths(i)) = originds(evalinds[j]);  
                  lengths(i)++;
                  cout << "n: " << acctotal << ", slice: " << i << ", ind: " << originds(evalinds[j]) << ", eval: " << sliceevals[evalinds[j]] << ", resnrm: " << sliceresnrms[evalinds[j]] << endl;
               }
               acctotal++;
            }
            j++;
         }
         break;
      }
   }

   for (int i = 0; i < nslices; i++) {
      tempvec.resize(lengths(i));
      tempvec = inds[i].head(lengths(i));
      inds[i] = tempvec;
   }
}

VectorXi sortinds(VectorXd xs) {
   int m = xs.size();

   VectorXi inds(m);

   int numless, equal = 0;

   for (int i = 0; i < m; i++) {
      numless = 0;
      for (int j = 0; j < m; j++) {
         if (xs[j] < xs[i]) {
            numless++;
         }
         else if (xs[j] == xs[i] && i != j) {
            if (i < j) {
               numless++; 
            }
         }
      }
      inds[numless] = i;
   }
   return inds;
}
