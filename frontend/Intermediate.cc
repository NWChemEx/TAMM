...
void generate_equations(CompilationUnit* const root,
...
void generate_equations_CompoundElement(const CompoundElement* const ce,
                                        Equations* const equations) {
...
void generate_equations_Element(Element* const element,
                                Equations* const equations) {
...
void generate_equations_DeclarationList(const DeclarationList* const decllist,
                                        Equations* const equations) {
  for (auto& decl : decllist->dlist)
    generate_equations_Declaration(decl, equations);
}

void generate_equations_Declaration(Declaration* const declaration,
                                    Equations* const equations) {
  if (RangeDeclaration* const rdecl =
    ...
    if (range_name == "O")
      rid = 0;
    else if (range_name == "V")
      rid = 1;
    else if (range_name == "N")
      rid = 2;
    equations->index_entries.push_back(
        new IndexEntry(idecl->index_name->name, rid));
  }

  else if (ArrayDeclaration* const adecl =
               dynamic_cast<ArrayDeclaration*>(declaration)) {
    TensorEntry* te = new TensorEntry(
        adecl->tensor_name->name,
        adecl->upper_indices.size() + adecl->lower_indices.size(),
        adecl->upper_indices.size());
    for (auto& ui : adecl->upper_indices)
      if (ui->name == "O")
        te->range_ids.push_back(0);
      else if (ui->name == "V")
        te->range_ids.push_back(1);
      else if (ui->name == "N")
        te->range_ids.push_back(2);
...
void get_index_ids(Equations* const equations, Array* const aref,
...
void generate_equations_Statement(Statement* const statement,
                                  Equations* const equations) {
  if (AssignStatement* const as = dynamic_cast<AssignStatement*>(statement))
    generate_equations_AssignStatement(as, equations);
}

void generate_equations_AssignStatement(const AssignStatement* const statement,
                                        Equations* const equations) {
  ...
  float alpha = 1.0;
  if (add_operators.size() == 1)
    if (add_operators.at(0) == "-") alpha = -1.0;
  ...
  if (num_rhs_arefs == 1) {
    ...
    equations->op_entries.push_back(
        new OpEntry(op_id, OpTypeAdd, aop, nullptr));
    op_id++;
    ...
    equations->op_entries.push_back(
        new OpEntry(op_id, OpTypeMult, nullptr, mop));
    op_id++;

  }

  else
    ;  /// Feature not implemented. print num_rhs_arefs
}
