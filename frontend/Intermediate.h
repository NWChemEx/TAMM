...
class IndexEntry {
 public:
  ...
  IndexEntry(const std::string index_name, const int range_id)
      : index_name(index_name), range_id(range_id) {}
  ~IndexEntry() {}
};
...
class TensorEntry {
 public:
  ...
  std::vector<int>
      range_ids; /*dimensions in terms of index into RangeEntry vector */
...
// //tamm_string_array collectExpIndices(Exp* exp, int* first_ref); //Get each
// index only once
