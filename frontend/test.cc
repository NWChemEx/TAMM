...
int main(int argc, const char* argv[]) {
  ...
  for (auto& ie : equations->index_entries) {
    ...
    std::cout << "At position " << i << " -> " << ient->index_name << " "
              << ient->range_id << std::endl;
    i++;
  }
  ...
  for (i = 0; i < equations->op_entries.size(); i++) {
    ...
    std::cout << std::fixed;
    if (oent->add != nullptr) {
      std::cout << getTensorName(tensor_entries, oent->add->tc) << ", "
                << getTensorName(tensor_entries, oent->add->ta) << ", "
                << oent->add->alpha << ", {";
      for (j = 0; j < oent->add->tc_ids.size(); j++)
        if (oent->add->tc_ids[j] != -1)
          std::cout << getIndexName(index_entries, oent->add->tc_ids[j]) << ",";
      std::cout << "}, {";
      for (j = 0; j < oent->add->ta_ids.size(); j++)
        if (oent->add->ta_ids[j] != -1)
          std::cout << getIndexName(index_entries, oent->add->ta_ids[j]) << ",";
      std::cout << "}";
    } else if (oent->mult != nullptr) {
      std::cout << getTensorName(tensor_entries, oent->mult->tc) << ", "
                << getTensorName(tensor_entries, oent->mult->ta) << ", "
                << getTensorName(tensor_entries, oent->mult->tb) << ", "
                << oent->mult->alpha << ", {";
      for (j = 0; j < oent->mult->tc_ids.size(); j++)
        if (oent->mult->tc_ids[j] != -1)
          std::cout << getIndexName(index_entries, oent->mult->tc_ids[j])
                    << ",";
      std::cout << "}, {";
      for (j = 0; j < oent->mult->ta_ids.size(); j++)
        if (oent->mult->ta_ids[j] != -1)
          std::cout << getIndexName(index_entries, oent->mult->ta_ids[j])
                    << ",";
      std::cout << "}, {";
      for (j = 0; j < oent->mult->tb_ids.size(); j++)
        if (oent->mult->tb_ids[j] != -1)
          std::cout << getIndexName(index_entries, oent->mult->tb_ids[j])
                    << ",";
      std::cout << "}";
    }
    std::cout << std::endl;
