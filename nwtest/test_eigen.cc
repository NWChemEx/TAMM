...
bool test_assign_no_n(tammx::ExecutionContext& ec,
  ...
  /// eigen
  tammx::Tensor<double> *ta_e1 = tamm_tensor_to_tammx_tensor(ec.pg(), &ta);
  auto al = tamm_label_to_tammx_label(alabels);
  ...
  eigen_tc += alpha * eigen_ta;

  std::cout << "eigen compute done\n";
  ...
  tamm_destroy(&tc1, &tc2, &ta);
  Tensor<double>::deallocate(e2tx_tc);
  return status;
}
