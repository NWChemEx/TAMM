...
typedef enum { TO, TV, TN, RANGE_UB } RangeType;
...
std::ostream& operator<<(std::ostream& os, const std::vector<T>& v) {
  os << "(";
  for (typename std::vector<T>::const_iterator itr = v.begin(); itr != v.end();
       ++itr)
    os << *itr << " ";
  os << ")";
  return os;
...
template <typename T>
bool compareVec(const std::vector<T>& v1, const std::vector<T>& v2) {
...
template <typename T>
int countParitySign(const std::vector<T>& v1, std::vector<T> v2) {
  int parity = 0;
  for (int i = 0; i < v1.size(); i++) {
    if (v1[i] != v2[i]) {
...
inline bool is_permutation(const std::vector<IndexName>& ids1,
  ...
  for (int i = 0; i < ids1.size(); i++) {
    if (sids2.find(ids1[i]) == sids2.end()) return false;
  }
  return true;
}
