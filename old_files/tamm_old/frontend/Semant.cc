...
void type_check(const CompilationUnit* const root, SymbolTable* const context) {
  for (auto& ce : root->celist) check_CompoundElement(ce, context);
}
...
void check_CompoundElement(const CompoundElement* const ce,
  ...
  for (auto& elem : element_list->elist) check_Element(elem, context);
}
...
void check_DeclarationList(const DeclarationList* const decllist,
                           SymbolTable* const context) {
  for (auto& decl : decllist->dlist) check_Declaration(decl, context);
}
...
bool check_tensor_usage(const index_list& def, const index_list& use) {
  ...
  for (int i = 0; i < def.size(); i++)
    if (def[i] != "N")
      if (def[i] != use[i]) return false;
  return true;
}

void check_index_reference(Identifier* const index,
                           SymbolTable* const context) {
...
void check_array_reference(Array* const aref, SymbolTable* const context) {
  ...
  if (tensor_entry == nullptr) {
    const std::string array_decl_error =
        "Tensor " + tensor_name + " is not defined";
  ...
  if (duplicate_indices) {
    ...
    const std::string duplicate_index_error =
        "Repetitive index in tensor reference: " + aref_text;
  ...
  if (TensorType* const t = dynamic_cast<TensorType*>(tensor_type)) {
    ...
    ;  ////Error: tensor_type cannot be of some other type
  ...
  if (!tensor_use) {
    const std::string aref_text = aref->getText();
    const std::string tensor_usage_error =
        "Tensor reference " + aref_text + " must have index structure of " +
        get_index_list_as_string(index_types) + " but is used as " +
        get_index_list_as_string(usage_types);
...
void check_AssignStatement(const AssignStatement* const statement,
  ...
  index_list non_summ_indices =
      get_non_summation_indices_from_expression(rhs_arefs);
  ...
  ///    TODO: If lhs array ref occurs on rhs (a=a+b*c) complain saying that
  ///    a+=b*c must be used instead
  //    if (exists_index(rhs_arefs->list,rhs_arefs->length,lhs_aref->list[0])){
  //        std::cerr << "Error at line " << s->u.AssignStmt.lhs->lineno << ":
  //        array " << lhs_aref->list[0] << " cannot be assigned after being
  //        previously referenced\n"; std::exit(EXIT_FAILURE);
  //    }
}

void check_range(const Identifier* const range_var,
                 SymbolTable* const context) {
  const std::string range_name = range_var->name;
  /// RANGE VARIABLE MAY or MAY NOT BE DEFINED
  //   if (context->get(range_name) == nullptr) {
  //         std::string range_var_error = "Range variable " + range_name + " is
  //         not defined"; Error(idecl->line, idecl->range_id->position,
  //         range_var_error);
  //    }
  const std::vector<std::string> allowed_ranges{"O", "V", "N"};
  if (std::find(allowed_ranges.begin(), allowed_ranges.end(), range_name) ==
      allowed_ranges.end()) {
    std::string range_error =
        "Range " + range_name + " is not supported. Can only be one of O, V, N";
...
void check_Declaration(Declaration* const declaration,
                       SymbolTable* const context) {
  if (RangeDeclaration* const rdecl =
          dynamic_cast<RangeDeclaration*>(declaration)) {
    const std::string range_var = rdecl->name->name;
    if (context->get(range_var) != nullptr) {
      std::string range_decl_error =
          "Range " + range_var + " is already defined";
  ...
  else if (IndexDeclaration* const idecl =
               dynamic_cast<IndexDeclaration*>(declaration)) {
    ...
    if (context->get(index_name) != nullptr) {
      std::string index_decl_error =
          "Index " + index_name + " is already defined";
  ...
  } else if (ArrayDeclaration* const adecl =
                 dynamic_cast<ArrayDeclaration*>(declaration)) {
    const std::string tensor_name = adecl->tensor_name->name;
    if (context->get(tensor_name) != nullptr) {
      std::string array_decl_error =
          "Tensor " + tensor_name + " is already defined";
    ...
    context->put(tensor_name, new Entry(new TensorType(adecl->upper_indices,
                                                       adecl->lower_indices)));
  }
}
...
void check_expression(Expression* const exp, SymbolTable* const context) {
  ...
  //                       << ": subexpressions of an addition must have equal
  //                       index sets\n";
  //             std::exit(EXIT_FAILURE);
  //         }
  ...
  //             std::cerr << "Error at line " << clno << ": summation index "
  //             << i <<
