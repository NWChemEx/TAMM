...
namespace ctce {
  ...
  double computeFactor(const std::vector<Integer> &ids) {
    ...
    if ((ids[0]==ids[1])&&(ids[1]==ids[2])) factor /= 6.0;
    else if ((ids[0]==ids[1])||(ids[1]==ids[2])) factor /= 2.0;
    if ((ids[3]==ids[4])&&(ids[4]==ids[5])) factor /= 6.0;
    else if ((ids[3]==ids[4])||(ids[4]==ids[5])) factor /= 2.0;
    return factor;
  }
  ...
  void computeEnergy(const std::vector<Integer>& rvec, const std::vector<Integer>& ovec,
    ...
    rh1 = rvec[3]; oh1 = ovec[3];
    rh2 = rvec[4]; oh2 = ovec[4];
    rh3 = rvec[5]; oh3 = ovec[5];
    int i=0;
          ...
          for (int t_h1=1; t_h1<=rh1; t_h1++) {
            ...
            for (int t_h2=1; t_h2<=rh2; t_h2++) {
              ...
              for (int t_h3=1; t_h3<=rh3; t_h3++) {
                ...
                denom = 1.0 / ( (denom_h1+denom_h2+denom_h3)-(denom_p4+denom_p5+denom_p6) );
                *energy1 += factor*denom*buf_double[i]*buf_double[i];
                *energy2 += factor*denom*buf_double[i]*(buf_double[i]+buf_single[i]);
                i++;
              }
            }
          }
