...
struct AddOpPlanBase {
    ...
    virtual void apply(const AddOpT& addop, ExecutionContext& ec,
                       ExecutionHW hw) = 0;
}; // AddOpPlanBase
...
struct FlatAddPlan : public AddOpPlanBase<T, LabeledTensorT1, LabeledTensorT2> {
    ...
    std::vector<TensorBase*> global_accumulates(
      const AddOpT& addop) const override {
        return {};
    }
    ...
    void apply(const AddOpT& addop, ExecutionContext& ec,
...
template<typename T, typename LabeledTensorT1, typename LabeledTensorT2>
struct GeneralFlatAddPlan
  : public AddOpPlanBase<T, LabeledTensorT1, LabeledTensorT2> {
    using AddOpT = AddOp<T, LabeledTensorT1, LabeledTensorT2>;
    std::vector<TensorBase*> global_writes(const AddOpT& addop) const override {
        if(addop.is_assign()) {
            return {addop.lhs().base_ptr()};
...
public:
    ...
    std::shared_ptr<Op> clone() const override {
        return std::shared_ptr<Op>(
          new AddOp<T, LabeledTensorT1, LabeledTensorT2>{*this});
    }

    void execute(ExecutionContext& ec,
                 ExecutionHW hw = ExecutionHW::CPU) override {
        ...
        if(rhs_.tensor().kind() != TensorBase::TensorKind::lambda &&
           ...
           lhs_.labels() == rhs_.labels() && !internal::is_slicing(lhs_) &&
           !internal::is_slicing(rhs_) &&
           lhs_.tensor().has_spin() == rhs_.tensor().has_spin() &&
           lhs_.tensor().execution_context()->pg() ==
             rhs_.tensor().execution_context()->pg() &&
           lhs_.tensor().distribution() == rhs_.tensor().distribution()) {
            plan_     = Plan::flat;
            plan_obj_ = std::make_shared<
              internal::FlatAddPlan<T, LabeledTensorT1, LabeledTensorT2>>();
            general_plan_obj_ =
              std::make_shared<internal::GeneralFlatAddPlan<T, LabeledTensorT1,
                                                            LabeledTensorT2>>();
        } else {
            plan_     = Plan::lhs;
            plan_obj_ = std::make_shared<
              internal::LHSAddPlan<T, LabeledTensorT1, LabeledTensorT2>>();
            general_plan_obj_ =
              std::make_shared<internal::GeneralLHSAddPlan<T, LabeledTensorT1,
                                                           LabeledTensorT2>>();
        }
...
protected:
    ...
    std::shared_ptr<
      internal::AddOpPlanBase<T, LabeledTensorT1, LabeledTensorT2>>
      plan_obj_;
    std::shared_ptr<
      internal::AddOpPlanBase<T, LabeledTensorT1, LabeledTensorT2>>
      general_plan_obj_;
...
template<typename T, typename LabeledTensorT1, typename LabeledTensorT2>
void FlatAddPlan<T, LabeledTensorT1, LabeledTensorT2>::apply(
  const AddOpT& addop, ExecutionContext& ec,
  ExecutionHW hw) {
...
template<typename T, typename LabeledTensorT1, typename LabeledTensorT2>
void LHSAddPlan<T, LabeledTensorT1, LabeledTensorT2>::apply(
  const AddOpT& addop, ExecutionContext& ec,
  ExecutionHW hw) {
    using T1          = typename LabeledTensorT1::element_type;
    using T2          = typename LabeledTensorT2::element_type;
    ...
    IndexLabelVec merged_alloc_labels = internal::merge_vector<IndexLabelVec>(
      lhs_alloc_lt.labels(), rhs_alloc_lt.labels());
    ...
    for(const auto& blockid : loop_nest) {
        ...
        if(tlb_valid && lhs_proc == me) {
            lambda(l_blockid, lhs_offset, r_blockid);
        }
    }
}

template<typename T, typename LabeledTensorT1, typename LabeledTensorT2>
void GeneralFlatAddPlan<T, LabeledTensorT1, LabeledTensorT2>::apply(
  const AddOpT& addop, ExecutionContext& ec,
  ExecutionHW hw) {
    using T1 = typename LabeledTensorT1::element_type;
    using T2 = typename LabeledTensorT2::element_type;
    ...
    for(size_t i = 0; i < pg_lhs_in_ec.size(); i++) {
        ...
        if(proc_me_in_ec == assigned_proc) {
            ...
            if(proc_me_in_ec == pg_lhs_in_ec[i]) {
                ...
                lhs_mem_region->get(Proc{i}, Offset{0}, Size{lhs_size},
                                    lhs_buf);
            }
            if(proc_me_in_ec == pg_rhs_in_ec[i]) {
                ...
                rhs_mem_region->get(Proc{i}, Offset{0}, Size{rhs_size},
                                    rhs_buf);
            }
            std::vector<size_t> lhs_dims{lhs_size};
            std::vector<size_t> rhs_dims{rhs_size};
            ...
            BlockAssignPlan::OpType optype =
              is_assign ? optype = BlockAssignPlan::OpType::set :
                          BlockAssignPlan::OpType::update;
            BlockAssignPlan plan{lhs_lt.labels(), rhs_lt.labels(), optype};
            plan.apply(lhs_span, alpha, rhs_span);
            if(proc_me_in_ec != pg_lhs_in_ec[i]) {
                ...
                auto* lhs_mem_region = lhs_tensor.memory_region();
                lhs_mem_region->put(Proc{i}, Offset{0}, Size{lhs_size},
                                    lhs_buf);
            }
...
template<typename T, typename LabeledTensorT1, typename LabeledTensorT2>
void GeneralLHSAddPlan<T, LabeledTensorT1, LabeledTensorT2>::apply(
  const AddOpT& addop, ExecutionContext& ec,
  ExecutionHW hw) {
    using T1        = typename LabeledTensorT1::element_type;
    using T2        = typename LabeledTensorT2::element_type;
        ...
        if(tlb_valid && lhs_lt.tensor().is_non_zero(l_blockid) &&
           rhs_lt.tensor().is_non_zero(r_blockid)) {
            ...
            if(lhs_owner_in_ec != Proc{-1}) {
                ...
                if(rhs_owner_in_ec != Proc{-1}) {
                    assigned_proc = rhs_owner_in_ec;
                } else {
