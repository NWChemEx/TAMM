...
public:
    ...
    template<typename T>
    BlockAssignPlan(const std::vector<T>& lhs_labels,
                    const std::vector<T>& rhs_labels, OpType optype) :
      optype_{optype},
      ...
      plan_{Plan::invalid} {
        ...
        if(plan_ == Plan::invalid) {
            prep_general_loop_plan(lhs_labels, rhs_labels);
        }
        if(plan_ == Plan::invalid) {
            prep_general_plan(lhs_labels, rhs_labels);
        }
    ...
    void apply_impl(BlockSpan<T1>& lhs, const BlockSpan<T2>& rhs) {
        ...
        switch(plan_) {
            ...
            case Plan::hptt:
                blockops::hptt::index_permute_hptt(lscale, lhs.buf(), 1,
                                                   rhs.buf(), ip_plan_.perm_,
                                                   rhs.block_dims());
                break;
            case Plan::index_permute_assign:
                blockops::cpu::index_permute_assign(
                  lhs.buf(), rhs.buf(), ip_plan_.perm_, lhs.block_dims());
                break;
            case Plan::index_permute_update:
                blockops::cpu::index_permute_update(
                  lhs.buf(), rhs.buf(), ip_plan_.perm_, lhs.block_dims());
                break;
            case Plan::ipgen_loop_assign:
                ipgen_loop_builder_.update_plan(lhs, rhs);
                blockops::cpu::ipgen_loop_assign(
                  lhs.buf(), ipgen_loop_builder_.u2ald()[0], rhs.buf(),
            ...
            case Plan::general_assign:
                blockops::cpu::ipgen_assign(
                  lhs.buf(), lhs.block_dims(), gen_plan_.dperm_map_,
    ...
    void apply_impl(BlockSpan<T1>& lhs, T1 rscale, const BlockSpan<T2>& rhs) {
        ...
        switch(plan_) {
            case Plan::flat_assign:
                blockops::cpu::flat_assign(lhs, rscale, rhs);
                break;
            case Plan::flat_update:
                blockops::cpu::flat_update(lhs, rscale, rhs);
                break;
            case Plan::hptt:
                blockops::hptt::index_permute_hptt(lscale, lhs.buf(), rscale,
                                                   rhs.buf(), ip_plan_.perm_,
                                                   rhs.block_dims());
                break;
            case Plan::index_permute_assign:
                blockops::cpu::index_permute_assign(lhs.buf(), rscale,
                                                    rhs.buf(), ip_plan_.perm_,
                                                    lhs.block_dims());
                break;
            case Plan::index_permute_update:
                blockops::cpu::index_permute_update(lhs.buf(), rscale,
                                                    rhs.buf(), ip_plan_.perm_,
                                                    lhs.block_dims());
                break;
            case Plan::ipgen_loop_assign:
                ipgen_loop_builder_.update_plan(lhs, rhs);
                blockops::cpu::ipgen_loop_assign(
                  lhs.buf(), ipgen_loop_builder_.u2ald()[0], rscale, rhs.buf(),
                  ipgen_loop_builder_.u2ald()[1],
                  ipgen_loop_builder_.unique_label_dims());
                break;
            case Plan::ipgen_loop_update:
    ...
    void apply(BlockSpan<T1>& lhs, Scalar rscale, const BlockSpan<T2>& rhs) {
        std::visit(
          overloaded{[&](auto alpha) {
              if constexpr(std::is_same_v<T1, T2> && std::is_convertible_v<decltype(alpha), T1>){
    ...
    template<typename T1, typename T2>
    void apply_impl(T1 lscale, BlockSpan<T1>& lhs, T2 rscale,
                    const BlockSpan<T2>& rhs) {
        EXPECTS(plan_ != Plan::invalid);
        ...
        switch(plan_) {
            ...
            case Plan::flat_update:
                blockops::cpu::flat_update(lscale, lhs, rscale, rhs);
                break;
            case Plan::hptt:
                blockops::hptt::index_permute_hptt(lscale, lhs.buf(), rscale,
                                                   rhs.buf(), ip_plan_.perm_,
            ...
            case Plan::index_permute_update:
                blockops::cpu::index_permute_update(lscale, lhs.buf(), rscale,
                                                    rhs.buf(), ip_plan_.perm_,
                                                    lhs.block_dims());
                break;
            ...
            case Plan::ipgen_loop_update:
                ipgen_loop_builder_.update_plan(lhs, rhs);
                blockops::cpu::ipgen_loop_update(
                  lscale, lhs.buf(), ipgen_loop_builder_.u2ald()[0], rscale,
            ...
            case Plan::general_update:
                blockops::cpu::ipgen_update(
                  lscale, lhs.buf(), lhs.block_dims(), gen_plan_.dperm_map_,
                  gen_plan_.dinv_perm_map_, rscale, rhs.buf(), rhs.block_dims(),
                  gen_plan_.sperm_map_, gen_plan_.unique_label_count_);
                break;
            case Plan::invalid: UNREACHABLE(); break;
    ...
    template<typename T1, typename T2>
    void apply(T1 lscale, BlockSpan<T1>& lhs, T2 rscale,
               const BlockSpan<T2>& rhs) {
        std::visit(
          overloaded{[&](auto beta, auto alpha) {
...
private:
    template<typename T>
    void prep_flat_plan(const std::vector<T>& lhs_labels,
                        const std::vector<T>& rhs_labels) {
        plan_ = optype_ == OpType::set ? Plan::flat_assign : Plan::flat_update;
        if(lhs_labels.size() != rhs_labels.size()) {
            plan_ = Plan::invalid;
            return;
        } else if(internal::unique_entries(lhs_labels).size() !=
                  lhs_labels.size()) {
            plan_ = Plan::invalid;
            return;
        } else if(!std::equal(lhs_labels.begin(), lhs_labels.end(),
                              rhs_labels.begin())) {
            plan_ = Plan::invalid;
            return;
        }
    ...
    template<typename T>
    void prep_hptt(const std::vector<T>& lhs_labels,
                   const std::vector<T>& rhs_labels) {
        prep_index_permute_plan(lhs_labels, rhs_labels);
        if(plan_ == Plan::invalid) {
    ...
    template<typename T>
    void prep_index_permute_plan(const std::vector<T>& lhs_labels,
                                 const std::vector<T>& rhs_labels) {
        plan_ = optype_ == OpType::set ? Plan::index_permute_assign :
                                         Plan::index_permute_update;
        if(lhs_labels.size() != rhs_labels.size()) {
            plan_ = Plan::invalid;
            return;
        } else if(internal::unique_entries(lhs_labels).size() !=
            ...
            plan_ = Plan::invalid;
            return;
        } else if(!std::is_permutation(lhs_labels.begin(), lhs_labels.end(),
                                       rhs_labels.begin())) {
            plan_ = Plan::invalid;
            return;
        }
    ...
    template<typename T>
    void prep_general_loop_plan(const std::vector<T>& lhs_labels,
                                const std::vector<T>& rhs_labels) {
        ipgen_loop_builder_ = blockops::cpu::IpGenLoopBuilder<2>(
          std::array<std::vector<T>, 2>{lhs_labels, rhs_labels});
        plan_ = optype_ == OpType::set ? Plan::ipgen_loop_assign :
                                         Plan::ipgen_loop_update;
    }

    template<typename T>
    void prep_general_plan(const std::vector<T>& lhs_labels,
                           const std::vector<T>& rhs_labels) {
        const auto& dlabel = lhs_labels;
        const auto& slabel = rhs_labels;
        plan_ =
          optype_ == OpType::set ? Plan::general_assign : Plan::general_update;
        ...
        gen_plan_.dperm_map_ =
          internal::perm_map_compute(unique_labels, dlabel);
        gen_plan_.sperm_map_ =
          internal::perm_map_compute(unique_labels, slabel);
        gen_plan_.dinv_perm_map_ =
          internal::perm_map_compute(dlabel, unique_labels);
    }
    ...
    std::string plan_str(Plan plan) {
        ...
        case Plan::hptt:
            return "hptt";
        case Plan::index_permute_assign:
            return "index_permute_assign";
        case Plan::index_permute_update:
            return "index_permute_update";
        case Plan::ipgen_loop_assign:
            return "ipgen_loop_assign";
        case Plan::ipgen_loop_update:
            return "ipgen_loop_update";
        case Plan::general_assign:
...
template<typename T1, typename T2>
void block_assign(BlockSpan<T1>& lhs, const std::vector<T2>& lhs_labels,
                  const BlockSpan<T1>& rhs, const std::vector<T2>& rhs_labels,
                  bool is_assign) {
    BlockAssignPlan::OpType optype = is_assign ?
...
void block_assign(BlockSpan<double>& lhs, const std::vector<int>& lhs_labels,
                  const BlockSpan<double>& rhs,
                  const std::vector<int>& rhs_labels, bool is_assign);
