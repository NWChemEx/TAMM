...
public:
  ...
  BlockBuffer(std::span<T> buf_span, IndexedTensor<T> indexedTensor, RuntimeEngine* re):
  ...
  BlockBuffer(Tensor<T> tensor, IndexVector blockid):
    storage_(tensor.block_size(blockid)), indexedTensor_{tensor, blockid} {
  ...
  BlockBuffer(const BlockBuffer& other):
    storage_{other.storage_}, indexedTensor_{other.indexedTensor_}, re_{other.re_} {
    ...
    if(!storage_.empty()) buf_span_ = std::span<T>{storage_.data(), storage_.size()};
    else buf_span_ = other.buf_span_;
  }
...
bool operator==(const BlockBuffer<T>& lhs, const BlockBuffer<T>& rhs) {
  return lhs.get_span().size() == rhs.get_span().size() &&
         std::equal(lhs.get_span().begin(), lhs.get_span().end(), rhs.get_span().begin());
}
