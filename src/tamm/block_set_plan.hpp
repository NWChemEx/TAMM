...
public:
  ...
  void apply(BlockSpan<T>& bs, T value) {
    switch(plan_) {
      ...
      case Plan::flat_update: blockops::cpu::flat_update(bs, value); break;
      case Plan::general_set:
        ipgen_loop_builder_.update_plan(bs);
        blockops::cpu::ipgen_loop_set(bs.buf(), ipgen_loop_builder_.u2ald()[0], value,
                                      ipgen_loop_builder_.unique_label_dims());
        break;
      case Plan::general_update:
        ipgen_loop_builder_.update_plan(bs);
        blockops::cpu::ipgen_loop_update(bs.buf(), ipgen_loop_builder_.u2ald()[0], value,
                                         ipgen_loop_builder_.unique_label_dims());
        break;
      default: UNREACHABLE();
    }
  ...
  void apply(T lscale, BlockSpan<T>& bs, T value) {
    switch(plan_) {
      ...
      case Plan::flat_update: blockops::cpu::flat_update(lscale, bs, value); break;
      case Plan::general_set: NOT_ALLOWED(); break;
      case Plan::general_update:
        ipgen_loop_builder_.update_plan(bs);
        blockops::cpu::ipgen_loop_update(lscale, bs.buf(), ipgen_loop_builder_.u2ald()[0], value,
                                         ipgen_loop_builder_.unique_label_dims());
        break;
      default: UNREACHABLE();
    }
...
private:
  ...
  void prep_general_plan(const std::vector<T>& labels, OpType optype) {
    // always valid
    ipgen_loop_builder_ = blockops::cpu::IpGenLoopBuilder<1>(std::array<std::vector<T>, 1>{labels});
    plan_               = (optype == OpType::set) ? Plan::general_set : Plan::general_update;
  }

  enum class Plan { invalid, flat_set, flat_update, general_set, general_update };
  OpType                             optype_;
  Plan                               plan_;
