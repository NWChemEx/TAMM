...
void ipgen_loop_set(TL* lbuf, const std::vector<int>& lld, TL value,
  ...
  if (ndim == 0) {
    *lbuf = value;
    ...
    for (int i0 = 0, i0doff = 0; i0 < unique_label_dims[0];
         ++i0, i0doff += lld[0]) {
      lbuf[i0doff] = value;
    }
  } else if (ndim == 2) {
    for (int i0 = 0, i0doff = 0; i0 < unique_label_dims[0];
         ++i0, i0doff += lld[0]) {
      for (int i1 = 0, i1doff = i0doff; i1 < unique_label_dims[1];
           ++i1, i1doff += lld[1]) {
        lbuf[i1doff] = value;
      }
    }
    ...
    for (int i0 = 0, i0doff = 0; i0 < unique_label_dims[0];
      ...
      for (int i1 = 0, i1doff = i0doff; i1 < unique_label_dims[1];
           ++i1, i1doff += lld[1]) {
        for (int i2 = 0, i2doff = i1doff; i2 < unique_label_dims[2];
             ++i2, i2doff += lld[2]) {
          lbuf[i2doff] = value;
        }
      }
    ...
    for (int i0 = 0, i0doff = 0; i0 < unique_label_dims[0];
      ...
      for (int i1 = 0, i1doff = i0doff; i1 < unique_label_dims[1];
        ...
        for (int i2 = 0, i2doff = i1doff; i2 < unique_label_dims[2];
             ++i2, i2doff += lld[2]) {
          for (int i3 = 0, i3doff = i2doff; i3 < unique_label_dims[3];
               ++i3, i3doff += lld[3]) {
            lbuf[i3doff] = value;
          }
        }
...
template <typename TL>
void ipgen_loop_update(TL lscale, TL* lbuf, const std::vector<int>& lld,
                       TL value, const std::vector<size_t>& unique_label_dims) {
  EXPECTS(lbuf != nullptr);
  EXPECTS(lld.size() == unique_label_dims.size());
  const size_t ndim = unique_label_dims.size();
  if (ndim == 0) {
    *lbuf = lscale * *lbuf + value;
  } else if (ndim == 1) {
    for (int i0 = 0, i0doff = 0; i0 < unique_label_dims[0];
         ++i0, i0doff += lld[0]) {
...
template <typename TL, typename TR>
void flat_update(TL lscale, BlockSpan<TL>& lhs, TL rscale,
                 const BlockSpan<TR>& rhs) {
  TL* lbuf = lhs.buf();
  const TR* rbuf = rhs.buf();
  const size_t num_elements = lhs.num_elements();
  for (size_t i = 0; i < num_elements; i++) {
    lbuf[i] = lscale * lbuf[i] + rscale * rbuf[i];
  }
}

template <typename TL, typename TR>
void flat_assign(BlockSpan<TL>& lhs, const Scalar& scale,
                 const BlockSpan<TR>& rhs) {
...
template <typename TL, typename TR>
void flat_update(BlockSpan<TL>& lhs, const Scalar& scale,
                 const BlockSpan<TR>& rhs) {
  std::visit(overloaded{[&](auto e) {
               if constexpr (std::is_assignable<TL&, decltype(e)>::value) {
...
void flat_lambda(Func&& func, BlockSpan<T>& block, BlockSpans&&... rest) {
  ...
  for (int i = 0; i < num_elements; i++) {
    std::forward<Func>(func)(block.buf()[i],
                             (std::forward<BlockSpans>(rest).buf()[i])...);
  }
}
...
inline size_t idx(int n, const size_t* id, const std::vector<size_t>& ldims,
                  const PermVector& p) {
  size_t idx = 0;
  for (int i = 0; i < n - 1; i++) {
    idx = (idx + id[p[i]]) * ldims[p[i + 1]];
  }
  if (n > 0) {
    idx += id[p[n - 1]];
  }
...
template <typename TL, typename TR>
void index_permute_assign(TL* lbuf, const TR* rbuf,
...
template <typename TL, typename TR>
void index_permute_assign(TL* lbuf, TL rscale, const TR* rbuf,
                          const PermVector& perm_to_dest,
                          const std::vector<size_t>& ldims) {
  EXPECTS(lbuf != nullptr && rbuf != nullptr);
  EXPECTS(perm_to_dest.size() == ldims.size());
  ...
  if (ndim == 0) {
    lbuf[0] = rscale * rbuf[0];
  } else if (ndim == 1) {
    for (size_t i = 0; i < ldims[0]; i++) {
      lbuf[i] = rscale * rbuf[i];
    }
  } else if (ndim == 2) {
    size_t i[2], c;
    for (c = 0, i[0] = 0; i[0] < ldims[0]; i[0]++) {
...
template <typename TL, typename TR>
void index_permute_update(TL* lbuf, const TR* rbuf,
                          const PermVector& perm_to_dest,
                          const std::vector<size_t>& ldims) {
  EXPECTS(lbuf != nullptr && rbuf != nullptr);
  EXPECTS(perm_to_dest.size() == ldims.size());
...
template <typename TL, typename TR>
void index_permute_update(TL* lbuf, TL rscale, const TR* rbuf,
                          const PermVector& perm_to_dest,
                          const std::vector<size_t>& ldims) {
  EXPECTS(lbuf != nullptr && rbuf != nullptr);
  EXPECTS(perm_to_dest.size() == ldims.size());
...
void index_permute_update(TL lscale, TL* lbuf, TL rscale, const TR* rbuf,
  ...
  if (ndim == 0) {
    lbuf[0] = lscale * lbuf[0] + rscale * rbuf[0];
  } else if (ndim == 1) {
    for (size_t i = 0; i < ldims[0]; i++) {
      lbuf[i] = lscale * lbuf[i] + rscale * rbuf[i];
    }
    ...
    for (c = 0, i[0] = 0; i[0] < ldims[0]; i[0]++) {
      for (i[1] = 0; i[1] < ldims[1]; i[1]++, c++) {
        lbuf[c] =
            lscale * lbuf[i] + rscale * rbuf[idx(2, i, ldims, perm_to_dest)];
      }
    }
  } else if (ndim == 3) {
    size_t i[3], c;
    for (c = 0, i[0] = 0; i[0] < ldims[0]; i[0]++) {
      for (i[1] = 0; i[1] < ldims[1]; i[1]++) {
        for (i[2] = 0; i[2] < ldims[2]; i[2]++, c++) {
          lbuf[c] =
              lscale * lbuf[i] + rscale * rbuf[idx(3, i, ldims, perm_to_dest)];
        }
      }
    }
  } else if (ndim == 4) {
    ...
    for (c = 0, i[0] = 0; i[0] < ldims[0]; i[0]++) {
      for (i[1] = 0; i[1] < ldims[1]; i[1]++) {
        for (i[2] = 0; i[2] < ldims[2]; i[2]++) {
          for (i[3] = 0; i[3] < ldims[3]; i[3]++, c++) {
            lbuf[c] = lscale * lbuf[i] +
                      rscale * rbuf[idx(4, i, ldims, perm_to_dest)];
          }
        }
      }
    }
...
inline size_t ipgen_idx(const std::vector<size_t>& index_vec,
...
template <typename TL, typename TR>
void ipgen_assign(TL* lbuf, const std::vector<size_t>& ddims,
                  const PermVector& dperm_map, const PermVector& dinv_perm_map,
                  TR* rbuf, const std::vector<size_t>& sdims,
...
void ipgen_assign(TL* lbuf, const std::vector<size_t>& ddims,
                  const PermVector& dperm_map, const PermVector& dinv_perm_map,
                  TL rscale, TR* rbuf, const std::vector<size_t>& sdims,
                  const PermVector& sperm_map, int unique_label_count) {
  std::vector<size_t> itrv(unique_label_count, 0);
  std::vector<size_t> endv(unique_label_count);
  endv = internal::perm_map_apply(ddims, dinv_perm_map);
...
template <typename TL, typename TR>
void ipgen_update(TL* lbuf, const std::vector<size_t>& ddims,
                  const PermVector& dperm_map, const PermVector& dinv_perm_map,
                  TR* rbuf, const std::vector<size_t>& sdims,
                  const PermVector& sperm_map, int unique_label_count) {
  std::vector<size_t> itrv(unique_label_count, 0);
...
template <typename TL, typename TR>
void ipgen_loop_assign(TL* lbuf, const std::vector<int>& lld, TR* rbuf,
                       const std::vector<int>& rld,
                       const std::vector<int>& unique_label_dims) {
  EXPECTS(lbuf != nullptr);
...
template <typename TL, typename TR>
void ipgen_loop_assign(TL* lbuf, const std::vector<int>& lld, TL rscale,
                       TR* rbuf, const std::vector<int>& rld,
                       const std::vector<int>& unique_label_dims) {
  EXPECTS(lbuf != nullptr);
  EXPECTS(rbuf != nullptr);
  EXPECTS(lld.size() == rld.size());
...
template <typename TL, typename TR>
void ipgen_loop_update(TL* lbuf, const std::vector<int>& lld, TR* rbuf,
                       const std::vector<int>& rld,
                       const std::vector<int>& unique_label_dims) {
  EXPECTS(lbuf != nullptr);
  EXPECTS(rbuf != nullptr);
...
template <typename TL, typename TR>
void ipgen_loop_update(TL* lbuf, const std::vector<int>& lld, TR rscale,
                       const TR* rbuf, const std::vector<int>& rld,
                       const std::vector<int>& unique_label_dims) {
  EXPECTS(lbuf != nullptr);
  EXPECTS(rbuf != nullptr);
  EXPECTS(lld.size() == rld.size());
...
template <typename TL, typename TR>
void ipgen_loop_update(TL lscale, TL* lbuf, const std::vector<int>& lld,
                       TL rscale, const TR* rbuf, const std::vector<int>& rld,
                       const std::vector<int>& unique_label_dims) {
  EXPECTS(lbuf != nullptr);
  EXPECTS(rbuf != nullptr);
  EXPECTS(lld.size() == rld.size());
...
void ipgen_loop_lambda(Func&& func,
                       const std::vector<size_t>& unique_label_dims,
                       const std::array<std::vector<size_t>, N>& blds,
                       Bufs... bufs) {
...
class IpGenLoopBuilder {
 public:
  ...
  IpGenLoopBuilder(const std::array<std::vector<T>, N>& labels_arr) {
    ...
    auto perm_dup_map_compute = [](const std::vector<T>& from,
  ...
  const std::array<std::vector<int>, N>& a2u_perm_map() const {
    return a2u_perm_map_;
  }
  int unique_label_count() const { return unique_label_count_; }
  const std::vector<int>& unique_label_dims() const {
    return unique_label_dims_;
  }
  ...
  void update_plan(BlockSpans&&... blocks) {
    ...
    auto exclusive_product = [](const std::vector<size_t>& vec) {
      ...
      std::partial_sum(vec.rbegin(), vec.rend(), ret.rbegin(),
                       std::multiplies<int>{});
    ...
    for (int i = 0; i < N; i++) {
      blds[i] = exclusive_product(bdims[i]);
    }
