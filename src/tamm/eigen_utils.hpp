...
void
patch_copy(std::vector<T> &sbuf, Eigen::Tensor<T, ndim, Eigen::RowMajor> &etensor,
...
template<typename T>
void
patch_copy(std::vector<T> &sbuf, Eigen::Tensor<T, 1, Eigen::RowMajor> &etensor,
...
template<typename T>
void
patch_copy(std::vector<T> &sbuf, Eigen::Tensor<T, 2, Eigen::RowMajor> &etensor,
           const std::vector<size_t> &block_dims,
           const std::vector<size_t> &block_offset, bool t2e=true) {
  size_t c = 0;
...
template<typename T>
void
patch_copy(std::vector<T> &sbuf, Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> &etensor,
           const std::vector<size_t> &block_dims,
           const std::vector<size_t> &block_offset, bool t2e=true) {
  size_t c = 0;
  for (size_t i= block_offset[0]; i < block_offset[0] + block_dims[0]; i++) {
...
template<typename T>
void
patch_copy(std::vector<T> &sbuf, Eigen::Tensor<T, 3, Eigen::RowMajor> &etensor,
           const std::vector<size_t> &block_dims,
           const std::vector<size_t> &block_offset, bool t2e=true) {
  size_t c = 0;
  for (size_t i= block_offset[0]; i < block_offset[0] + block_dims[0]; i++) {
...
patch_copy(std::vector<T> &sbuf, Eigen::Tensor<T, 4, Eigen::RowMajor> &etensor,
  ...
  for (size_t i= block_offset[0]; i < block_offset[0] + block_dims[0]; i++) {
    for (size_t j= block_offset[1]; j < block_offset[1] + block_dims[1];
      ...
      for (size_t k= block_offset[2]; k < block_offset[2] + block_dims[2];
           k++) {
        for (size_t l= block_offset[3]; l < block_offset[3] + block_dims[3];
             l++, c++) {
