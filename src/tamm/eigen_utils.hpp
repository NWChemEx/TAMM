...
void patch_copy(std::vector<T>& sbuf, Eigen::Tensor<T, ndim, Eigen::RowMajor>& etensor,
                const std::array<int, ndim>& block_dims, const std::array<int, ndim>& block_offset,
                bool t2e = true) {
  assert(0);
}

template<typename T>
void patch_copy(std::vector<T>& sbuf, Eigen::Matrix<T, 1, Eigen::Dynamic>& etensor,
                const std::vector<size_t>& block_dims, const std::vector<size_t>& block_offset,
                bool t2e = true) {
  size_t c = 0;
  for(size_t i = block_offset[0]; i < block_offset[0] + block_dims[0]; i++, c++) {
    if(t2e) etensor(i) = sbuf[c];
    else sbuf[c] = etensor(i);
  }
}

template<typename T>
void patch_copy(std::vector<T>& sbuf, Eigen::Tensor<T, 1, Eigen::RowMajor>& etensor,
                const std::vector<size_t>& block_dims, const std::vector<size_t>& block_offset,
                bool t2e = true) {
  size_t c = 0;
  for(size_t i = block_offset[0]; i < block_offset[0] + block_dims[0]; i++, c++) {
...
template<typename T>
void patch_copy(std::vector<T>& sbuf, Eigen::Tensor<T, 2, Eigen::RowMajor>& etensor,
                const std::vector<size_t>& block_dims, const std::vector<size_t>& block_offset,
                bool t2e = true) {
  size_t c = 0;
  for(size_t i = block_offset[0]; i < block_offset[0] + block_dims[0]; i++) {
    for(size_t j = block_offset[1]; j < block_offset[1] + block_dims[1]; j++, c++) {
      if(t2e) etensor(i, j) = sbuf[c];
      else sbuf[c] = etensor(i, j);
    }
  }
}
...
template<typename T>
void patch_copy(std::vector<T>& sbuf, Eigen::Tensor<T, 3, Eigen::RowMajor>& etensor,
                const std::vector<size_t>& block_dims, const std::vector<size_t>& block_offset,
                bool t2e = true) {
  size_t c = 0;
  for(size_t i = block_offset[0]; i < block_offset[0] + block_dims[0]; i++) {
    for(size_t j = block_offset[1]; j < block_offset[1] + block_dims[1]; j++) {
      for(size_t k = block_offset[2]; k < block_offset[2] + block_dims[2]; k++, c++) {
        if(t2e) etensor(i, j, k) = sbuf[c];
        else sbuf[c] = etensor(i, j, k);
      }
    }
  }
}
...
template<typename T>
void patch_copy(std::vector<T>& sbuf, Eigen::Tensor<T, 4, Eigen::RowMajor>& etensor,
                const std::vector<size_t>& block_dims, const std::vector<size_t>& block_offset,
                bool t2e = true) {
  size_t c = 0;
  for(size_t i = block_offset[0]; i < block_offset[0] + block_dims[0]; i++) {
    for(size_t j = block_offset[1]; j < block_offset[1] + block_dims[1]; j++) {
      for(size_t k = block_offset[2]; k < block_offset[2] + block_dims[2]; k++) {
        for(size_t l = block_offset[3]; l < block_offset[3] + block_dims[3]; l++, c++) {
          if(t2e) etensor(i, j, k, l) = sbuf[c];
          else sbuf[c] = etensor(i, j, k, l);
        }
      }
    }
  }
...
inline Matrix eigen_tensor_to_matrix(Tensor2D& eig_tensor) {
  ...
  for(int i = 0; i < eig_mat.rows(); i++)
    for(int j = 0; j < eig_mat.cols(); j++) eig_mat(i, j) = eig_tensor(i, j);
  return eig_mat;
}

template<typename T, int ndim>
Eigen::Tensor<T, ndim, Eigen::RowMajor> tamm_to_eigen_tensor(const tamm::Tensor<T>& tensor) {
...
tamm_to_eigen_matrix(const tamm::Tensor<T>& tensor) {
  Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> etensor;
...
// 1D case
template<typename T>
void eigen_to_tamm_tensor(tamm::Tensor<T>& tensor, Eigen::Matrix<T, 1, Eigen::Dynamic>& etensor) {
  for(const auto& blockid: tensor.loop_nest()) {
    const tamm::TAMM_SIZE size = tensor.block_size(blockid);
    std::vector<T>        buf(size);
    // tensor.get(blockid, buf);
...
template<typename T>
void eigen_to_tamm_tensor_acc(tamm::Tensor<T>&                      tensor,
...
void eigen_matrix_to_tamm(matrix_type&& mat, tamm::Tensor<T>& t) {
  ...
  if constexpr(nmodes == 1) { // Actually a vector
    ...
    for(long int i = 0; i < nrows; ++i)
      for(long int j = 0; j < ncols; ++j) t1(i, j) = mat(i, j);
...
template<typename T>
tamm::Tensor<T> retile_rank2_tensor(tamm::Tensor<T>& tensor, const tamm::TiledIndexSpace& t1,
                                    const tamm::TiledIndexSpace& t2) {
