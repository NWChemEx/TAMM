...
VectorXd getshifts(MPI_Comm comm, MatrixXd &H, MatrixXd &S, int nev, int nshifts, int *maxcnt) 
{
   ...
   double sigma = 0.0;
   int64_t mid  = std::floor( (double)nlan / 2. );
   ...
   // Sort adjacent differences
   std::sort( gaps.begin(), gaps.end() );

   // Get median
   sigma = std::max( sigma, gaps[mid] );
   sigma = sqrt(sigma*sigma/2.0);

   // define DOS parameter 
   DensityOfStates DOS;
   int nSamp = 1; //for now
   double exp_fact  = 2. * sigma * sigma;
...
*/
   ...
   int nevloc = std::ceil(nev*1.01/nshifts);
