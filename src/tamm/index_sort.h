 ...
 *
 * @file index_sort
 * @brief Wrapper for index permutation routines. The implementation of these
 * methods can be performed using optimized libraries.
 *
 */
...
/**
 * @brief Scale elements in source 2-d array and assign to target 2-d array,
 * with permutation (p1,p2).
 *
 * @param[in] sbuf Pointer to source buffer
 * @param[in] dbuf Pointer to target buffer
...
void index_sort(const double* sbuf, double* dbuf, int ndim, const size_t* sizes,
    ...
    bool val = std::is_convertible<T, double>::value;
    EXPECTS(val);
    EXPECTS(sbuf);
    EXPECTS(dbuf);
    EXPECTS(ndim >= 0);
    ...
    if(ndim == 0) {
        *dbuf = *sbuf * alpha;
        ...
        index_sort_4(sbuf, dbuf, sizes[0], sizes[1], sizes[2], sizes[3],
 ...
 * @endcode
 */
void copy_add(const double* sbuf, double* dbuf, size_t sz, double alpha);
...
void index_sortacc(const double* sbuf, double* dbuf, int ndim,
    ...
    if(ndim == 0) {
        *dbuf += *sbuf * alpha;
        ...
        index_sortacc_2(sbuf, dbuf, sizes[0], sizes[1], perm[0], perm[1],
                        alpha);
    } else if(ndim == 3) {
        index_sortacc_3(sbuf, dbuf, sizes[0], sizes[1], sizes[2], perm[0],
                        perm[1], perm[2], alpha);
    } else if(ndim == 4) {
