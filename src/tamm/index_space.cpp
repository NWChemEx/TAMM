...
// Ctors
IndexSpace::IndexSpace(const IndexVector& indices, const NameToRangeMap& named_subspaces,
                       const AttributeToRangeMap<Spin>&    spin,
                       const AttributeToRangeMap<Spatial>& spatial):
  impl_{std::make_shared<RangeIndexSpaceImpl>(indices, named_subspaces, spin, spatial)
...
IndexSpace::IndexSpace(const std::vector<IndexSpace>& spaces, const std::vector<std::string>& names,
                       const NameToRangeMap&                                  named_subspaces,
                       const std::map<std::string, std::vector<std::string>>& subspace_references):
  impl_{std::make_shared<AggregateSpaceImpl>(spaces, names, named_subspaces, subspace_references)} {
  hash_value_ = impl_->hash();
  impl_->set_weak_ptr(impl_);
}
...
IndexSpace::IndexSpace(const std::vector<TiledIndexSpace>&      indep_spaces,
  ...
  impl_{std::make_shared<DependentIndexSpaceImpl>(indep_spaces, dep_space_relation)} {
  hash_value_ = impl_->hash();
  impl_->set_weak_ptr(impl_);
}
...
IndexSpace::IndexSpace(const std::vector<TiledIndexSpace>&      indep_spaces,
                       ...
                       const std::map<IndexVector, IndexSpace>& dep_space_relation):
  impl_{std::make_shared<DependentIndexSpaceImpl>(indep_spaces, ref_space, dep_space_relation)} {
  hash_value_ = impl_->hash();
  impl_->set_weak_ptr(impl_);
}
...
IndexSpace::IndexSpace(const std::vector<TiledIndexSpace>& indep_spaces,
  ...
  impl_       = std::make_shared<DependentIndexSpaceImpl>(indep_spaces, ref_space, ret);
  hash_value_ = impl_->hash();
  impl_->set_weak_ptr(impl_);
}
...
const std::map<IndexVector, IndexSpace>& IndexSpace::map_tiled_index_spaces() const {
...
std::size_t IndexSpace::max_num_indices() const { return impl_->max_num_indices(); }
...
SpatialAttribute IndexSpace::get_spatial() const { return impl_->get_spatial(); }

const NameToRangeMap& IndexSpace::get_named_ranges() const { return impl_->get_named_ranges(); }

IndexSpace IndexSpace::root_index_space() const { return impl_->root_index_space(); }

// Comparison operator implementations
bool operator==(const IndexSpace& lhs, const IndexSpace& rhs) { return lhs.is_identical(rhs); }

bool operator<(const IndexSpace& lhs, const IndexSpace& rhs) { return lhs.is_less_than(rhs); }

bool operator!=(const IndexSpace& lhs, const IndexSpace& rhs) { return !(lhs == rhs); }
