...
public:
   ...
   */
  IndexSpace(const std::initializer_list<Index>& indices): IndexSpace{indices, {}, {}, {}} {}
  ...
  IndexSpace(const Range& range, const NameToRangeMap& named_subspaces = {},
    ...
    IndexSpace{construct_index_vector(range), named_subspaces, spin, spatial} {}
   ...
   */
  IndexSpace(const IndexVector& indices, const NameToRangeMap& named_subspaces = {},
             const AttributeToRangeMap<Spin>&    spin    = {},
             const AttributeToRangeMap<Spatial>& spatial = {});
   ...
   */
  IndexSpace(const IndexSpace& is, const Range& range, const NameToRangeMap& named_subspaces = {});
  /**
   ...
   */
  IndexSpace(const std::vector<IndexSpace>& spaces, const std::vector<std::string>& names = {},
             const NameToRangeMap&                                  named_subspaces     = {},
   ...
   */
  IndexSpace(const std::vector<TiledIndexSpace>& indep_spaces, const IndexSpace& ref_space,
             const std::map<IndexVector, IndexSpace>& dep_space_relation);
  /**
   * @brief Construct a new (Dependent) IndexSpace object using a vector of
   * dependent IndexSpaces
   ...
   */
  bool is_identical(const IndexSpace& rhs) const { return (hash_value_ == rhs.hash()); }
   ...
   * later.
   */
  bool is_compatible(const IndexSpace& rhs) const { return is_identical(rhs); }
