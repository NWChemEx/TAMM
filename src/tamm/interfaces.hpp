...
class Cloneable {
public:
  std::unique_ptr<Base> clone() const& { return std::unique_ptr<Base>{clone_impl()}; }
  std::unique_ptr<Base> clone() && { return std::unique_ptr<Base>{std::move(*this).clone_impl()}; }
...
public:
  ...
  std::unique_ptr<This> clone() && {
    return std::unique_ptr<This>{static_cast<This*>(std::move(*this).clone_impl())};
  }
