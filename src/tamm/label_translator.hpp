...
public:
  ...
  LabelTranslator(const IndexLabelVec& from, const IndexLabelVec& to): from_{from}, to_{to} {
    EXPECTS(from.size() == to.size());
    if(from == to) { plan_ = Plan::identical; }
    else if(internal::is_dense_labels(from) && internal::is_dense_labels(to)) {
      if(is_dense_subspaces()) {
        plan_ = Plan::dense;
        IndexVector from_blockid(from.size(), 0);
        auto [to_blockid, is_valid] =
          internal::translate_blockid_if_possible(from_blockid, from_, to_);
        EXPECTS(is_valid);
        dense_plan_info_ = DensePlanInfo(to_blockid);
      }
      else { plan_ = Plan::independent; }
    }
    else {
      plan_              = Plan::general;
  ...
  std::pair<std::vector<T>, bool> apply(const std::vector<T>& from_blockid) {
    ...
    if(plan_ == Plan::identical) { return {from_blockid, is_valid}; }
      ...
      for(size_t i = 0; i < from_blockid.size(); i++) {
        to_blockid[i] =
          from_[i].tiled_index_space().translate(from_blockid[i], to_[i].tiled_index_space());
      }
    }
    else {
      ...
      for(size_t i = 0; i < from_.size(); i++) {
        if(!from_[i].tiled_index_space().is_compatible_with(to_[i].tiled_index_space())) {
      ...
      for(size_t i = 0; i < compute_order.size(); i++) {
        ...
        EXPECTS(
          from_[cur_pos].tiled_index_space().is_compatible_with(to_[cur_pos].tiled_index_space()));

        auto [to_id, valid] = from_[cur_pos].tiled_index_space().translate_if_possible(
          from_blockid[cur_pos], from_indep_vec, to_[cur_pos].tiled_index_space(), to_indep_vec);

        if(!valid) { return {to_blockid, false}; }
        else { to_blockid[compute_order[i]] = to_id; }
      }
    }
...
private:
  ...
  struct GeneralPlanInfo {
    ...
    GeneralPlanInfo(const IndexLabelVec& from_labels, const IndexLabelVec& to_labels) {
