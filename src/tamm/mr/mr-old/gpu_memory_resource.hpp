...
private:
   ...
   */
  void* do_allocate(std::size_t bytes, gpu_stream_view stream) override {
    void* ptr{nullptr};
#if defined(USE_CUDA)
   ...
   */
  void do_deallocate(void* ptr, std::size_t bytes, gpu_stream_view stream) override {
#if defined(USE_CUDA)
      std::cout << "calling cudaFree to allocate bytes: " << ptr << ", " << bytes << std::endl;
    cudaFree(ptr);
#elif defined(USE_HIP)
    hipFree(ptr);
#elif defined(USE_DPCPP)
