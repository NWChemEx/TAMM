...
public:
   ...
   */
  explicit pool_memory_resource(Upstream* upstream_mr, gpu_stream_view gpuStream,
    ...
    std::cout << "pool_memory_resource ctor : " << &gpuStream << std::endl;
...
protected:
   ...
   */
  block_type try_to_expand(std::size_t try_size, std::size_t min_size, gpu_stream_view stream) {
   ...
   */
  void initialize_pool(gpu_stream_view gpuStream, std::optional<std::size_t> initial_size,
                       std::optional<std::size_t> maximum_size) {
    auto const try_size = [&]() {
      if(not initial_size.has_value()) {
        std::size_t free{}, total{};
	tamm::gpuMemGetInfo(&free, &total);
    ...
    if(try_size > 0) {
      ...
      std::cout << "1. initialize_pool() :" << &gpuStream << std::endl;
      this->insert_block(block, gpuStream);
      std::cout << "2. initialize_pool() :" << &gpuStream << std::endl;
    }
  }
   ...
   */
  std::optional<block_type> block_from_upstream(std::size_t size, gpu_stream_view stream) {
    if(size == 0) { return {}; }
