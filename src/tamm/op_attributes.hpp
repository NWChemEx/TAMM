...
struct TensorInfo {
  ...
  TensorInfo(const std::string& name, TensorVariant tensor, const IndexLabelVec& ilv, ElType eltype,
             const Scalar& scale, bool is_intermediate):
    name_{name},
    tensor_{tensor},
    ilv_{ilv},
    eltype_{eltype},
  ...
  std::string to_string() const {
    std::string result = "";
    result += "tensor_name = " + name_ + " tensor_type = " + tensor_.to_string() +
              " el_type = " + eltype_to_string(eltype_) +
              " lbl_count = " + std::to_string(ilv_.size()) + " scale = " + scale_.to_string();
    return result;
  }
  ...
  std::string op_string(SymbolTable& symbol_table, bool print_scale = true) const {
    ...
    for(const auto& lbl: ilv_) {
      ...
      if(it == symbol_table.end()) {
        ...
        if(lbl.label_str() == "") { temp_lbl_str = "_lbl_" + std::to_string(lbl.label()); }
        else { temp_lbl_str = lbl.label_str(); }
        result += temp_lbl_str;
      }
      else {
        ...
        if(!lbl.secondary_labels().empty()) {
          ...
          for(const auto& dep_lbl: lbl.secondary_labels()) {
            EXPECTS(symbol_table.find(dep_lbl.get_symbol_ptr()) != symbol_table.end());
...
struct BinarizedOp {
  ...
  BinarizedOp(const TensorInfo& lhs, const TensorInfo& rhs1, const TensorInfo& rhs2,
              const Scalar& scale, BinOpType optype, bool is_assign = true):
    lhs_{lhs}, rhs1_{rhs1}, rhs2_{rhs2}, scale_{scale}, optype_{optype}, is_assign_{is_assign} {}
  ...
  std::string to_string() const {
    ...
    if(optype_ == BinOpType::addop) result += " + ";
    else if(optype_ == BinOpType::multop) result += " * ";
    if(optype_ != BinOpType::setop) {
      result += "( rhs2 = " + rhs2_.to_string() + " * l_scale = " + scale_.to_string() + " )";
    }
  ...
  std::string op_string(SymbolTable& symbol_table) const {
    ...
    if(scale_.value() != Scalar{lhs_.eltype_}.value()) { result += scale_.to_string() + " * "; }

    if(optype_ == BinOpType::setop) { result += rhs1_op_str + ";"; }

    if(optype_ == BinOpType::addop) {
      result += rhs1_op_str + ";\n";
      result += lhs_op_str + " += " + rhs2_op_str + ';';
    }
    else if(optype_ == BinOpType::multop) { result += rhs1_op_str + " * " + rhs2_op_str + ';'; }
...
public:
  ...
  friend void swap(AttributeInfo<T>& first, AttributeInfo<T>& second) noexcept {
    using std::swap;
...
public:
  ...
  friend void swap(AttributeInfo<InfoType>& first, AttributeInfo<InfoType>& second) noexcept {
    using std::swap;
    swap(first.info_, second.info_);
  }
...
class NameAttribute:
  public InheritWithCloneable<NameAttribute, OpAttribute>,
