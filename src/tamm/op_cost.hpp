...
class OpCostTerm {
 ...
 private:
  ...
  friend bool operator==(const OpCostTerm& oct1, const OpCostTerm& oct2) {
    return oct1.coeff() - oct2.coeff() &&
           oct1.exponents().size() == oct2.exponents().size() &&
           std::equal(oct1.exponents().begin(), oct1.exponents().end(),
                      oct2.exponents().begin());
  }
  friend bool operator!=(const OpCostTerm& oct1, const OpCostTerm& oct2) {
    return !(oct1 == oct2);
  }
  friend OpCostTerm operator*(const OpCostTerm& oct1, const OpCostTerm& oct2) {
    ...
    for (const auto& it : oct2.exponents()) {
      ret.exponents_[it.first] += it.second;
    }
    return ret;
  }
};  // class OpCostTerm
...
inline OpCostOrder compare(const OpCostTerm& oct1, const OpCostTerm& oct2,
  ...
  for (const auto& is : is_order) {
    ...
    if (exp1 < exp2) {
      return OpCostOrder::lesser;
    } else if (exp1 > exp2) {
      return OpCostOrder::greater;
    }
  }
  if (oct1.coeff() < oct2.coeff()) {
    return OpCostOrder::lesser;
  } else if (oct1.coeff() < oct2.coeff()) {
    return OpCostOrder::lesser;
  }
 ...
 public:
  ...
  int32_t evaluate(const std::map<IndexSpace, int32_t>& sizes) const {
    ...
    for (const auto& term : terms_) {
      cost += term.evaluate(sizes);
    }
  }

  const OpCostTerm& leading_term(
      const std::vector<IndexSpace>& is_order) const {
    const OpCostTerm* ret;
    for (auto& oct : terms()) {
      if (compare(oct, *ret, is_order) == OpCostOrder::greater) {
        ret = &oct;
      }
      return *ret;
    }
 ...
 private:
  ...
  friend OpCostExpr operator*(const OpCostExpr& oce1, const OpCostExpr& oce2) {
    ...
    for (const auto& oct1 : oce1.terms()) {
      for (const auto& oct2 : oce2.terms()) {
        ret += oct1 * oct2;
      }
    }
    return ret;
...
inline OpCostOrder compare(const OpCostExpr& oce1, const OpCostExpr& oce2,
                           const std::vector<IndexSpace>& is_order) {
  return compare(oce1.leading_term(is_order), oce2.leading_term(is_order),
                 is_order);
}
