...
public:
  ...
  template <typename T> 
  void report_to_json(const Tensor<T> &tensor, std::string eq_type, std::string eqs_name, json& output, bool use_opmin = true) {
    ...
    for (size_t update_idx = 0; update_idx < updates.size(); update_idx++) {
      ...
      auto canonicalized_ops =
          new_ops::CanonicalizeVisitor::canonicalize_ops(*update.op_);
      ...
      for (size_t i = 0; i < canonicalized_ops.size(); i++) {
        ...
        op_cost = get_op_cost(op->clone(), tensor(lhs_labels),
                                     update.is_update_, use_opmin);
        mem_cost = 
            get_op_mem_cost(op->clone(), tensor(lhs_labels), use_opmin);
        
        output[eq_type][eqs_name][std::to_string(update_idx)]["op_cost"] = op_cost;
        output[eq_type][eqs_name][std::to_string(update_idx)]["mem_cost"] = std::to_string(mem_cost / std::pow(2, 30)) + " GBs";
  ...
  void print_tensor_execution_report(const Tensor<T> &tensor, bool use_opmin = true) {
    ...
    for (size_t update_idx = 0; update_idx < updates.size(); update_idx++) {
      ...
      for (size_t i = 0; i < canonicalized_ops.size(); i++) {
        ...
        std::cout << "Update " << update_idx << " to Tensor " << symbol_table_[tensor.get_symbol_ptr()] << "\n";
  ...
  auto get_total_op_cost(const Tensor<T> &tensor, bool use_opmin = false) {
    ...
    for (size_t i = 0; i < updates.size(); i++) {
      ...
      for (size_t i = 0; i < canonicalized_ops.size(); i++) {
        ...
        total_op_cost += get_op_cost(op->clone(), tensor(lhs_labels),
                                     update.is_update_, use_opmin);
        max_mem_cost = std::max(
            max_mem_cost,
            get_op_mem_cost(op->clone(), tensor(lhs_labels), use_opmin));
      }
    }
  ...
  template <typename T>
  double get_op_cost(std::unique_ptr<new_ops::Op> op,
                     const LabeledTensor<T> &labeled_tensor, bool is_update = true,
                     bool use_opmin = false) {
    std::unique_ptr<new_ops::Op> optree = op->clone();
    optree->set_attribute<new_ops::NeededLabelsAttribute>(
        labeled_tensor.labels());
    if (use_opmin) {
  ...
  template <typename T>
  double get_op_mem_cost(std::unique_ptr<new_ops::Op> op,
                         const LabeledTensor<T> &labeled_tensor,
                         bool use_opmin = false) {
  ...
  void print_op_binarized(const new_ops::LTOp &lhs_ltop,
                          const std::unique_ptr<new_ops::Op> &in_op,
                          bool use_opmin = false) {
    ...
    for (size_t i = 0; i < canonicalized_ops.size(); i++) {
      ...
      binops.pop_back();
      new_ops::TensorInfo new_lhs{
          symbol_table_[lhs_ltop.tensor().get_symbol_ptr()],
          lhs_ltop.tensor(),
          lhs_ltop.labels(),
...
public:
    ...
    OpExecutor(Scheduler& sch, SymbolTable& symbol_table) :
      sch_{sch}, symbol_table_{symbol_table} {}
    ...
    void execute(TensorVariant tensor_variant, bool use_opmin = false,
                 ExecutionHW execute_on = ExecutionHW::CPU,
                 bool profile = false) {
      std::visit(overloaded{[&](const auto &tensor) {
                   execute(tensor, use_opmin, execute_on, profile);
                 }},
                 tensor_variant.value());
    }

    template <typename T>
    void execute(Tensor<T> tensor, bool use_opmin = false,
                 ExecutionHW execute_on = ExecutionHW::CPU,
                 bool profile = false) {
      // check if the tensor is allocated
      if(!tensor.is_allocated()) {
        sch_.allocate(tensor);
      }
      ...
      for (size_t i = tensor.version(); i < updates.size(); i++) {
        ...
        for (size_t i = 0; i < canonicalized_ops.size(); i++) {
          ...
          if (!use_old_lhs) {
            auto it = std::find(lhs_labels.begin(), lhs_labels.end(),
                                label_pair.first);
            EXPECTS(it != lhs_labels.end());
            auto index = std::distance(lhs_labels.begin(), it);
            lhs_labels[index] = label_pair.second;
          ...
          // check if used tensors are allocated and have any updates
          {
            op->accept(tensor_info);
            auto used_tensors =
                op->get_attribute<new_ops::UsedTensorInfoAttribute>().get();

            for (auto &tens_info : used_tensors) {
              if (!tens_info.tensor_.is_allocated()) {
                tens_info.tensor_.allocate(sch_);
              }
          ...
          if (use_opmin) {
            OpMin opmin{symbol_table_};
            auto optimized_op =
                opmin.optimize_all(new_ltop, *op, !update.is_update_);
            binops = new_ops::BinarizeOpsVisitor::binarize_op(*optimized_op,
                                                              symbol_table_);
          } else {
            binops =
                new_ops::BinarizeOpsVisitor::binarize_op(*op, symbol_table_);
          }
          ...
          for (auto &[key, tens] : inter_tensors) {
            tens.allocate(sch_);
          }
    ...
    template <typename T> void opmin_execute(Tensor<T> &tensor) {
      execute(tensor, true);
    }
    ...
    void construct_op(const new_ops::BinarizedOp& binop) {
        ...
        std::visit(
          overloaded{[&](auto lhs_t, auto rhs1_t, auto rhs2_t, auto alpha,
                         auto rhs1_alpha, auto rhs2_alpha) {
    ...
    void construct_inter(const std::vector<new_ops::BinarizedOp>& binops,
        ...
        for(const new_ops::BinarizedOp& op : binops) {
            if(op.lhs_.is_intermediate_ &&
               inter_tensors.find(op.lhs_.name_) == inter_tensors.end()) {
                inter_tensors[op.lhs_.name_] = op.lhs_.tensor_;
            }

            if(op.rhs1_.is_intermediate_ &&
               inter_tensors.find(op.rhs1_.name_) == inter_tensors.end()) {
                inter_tensors[op.rhs1_.name_] = op.rhs1_.tensor_;
            }

            if(op.rhs2_.is_intermediate_ &&
               inter_tensors.find(op.rhs2_.name_) == inter_tensors.end()) {
    ...
    template <typename T>
    void pretty_print_binarized(const Tensor<T> &tensor,
                                bool use_opmin = false, 
      ...
      for (size_t i = start_update; i < updates.size(); i++) {
        ...
        for (size_t i = 0; i < canonicalized_ops.size(); i++) {
          ...
          if (use_opmin) {
            OpMin opmin{symbol_table_};
            auto optimized_op =
                opmin.optimize_all(new_ltop, *update.op_, !update.is_update_);
            binops = new_ops::BinarizeOpsVisitor::binarize_op(*optimized_op,
                                                              symbol_table_);
          } else {
            binops =
    ...
    void print_op_cost(const Tensor<T> &tensor, bool use_opmin = false) {
      ...
      for (size_t i = 0; i < updates.size(); i++) {
        ...
        for (size_t i = 0; i < canonicalized_ops.size(); i++) {
          ...
          total_op_cost += get_op_cost(std::move(op), tensor(lhs_labels), update.is_update_, use_opmin);
        }
    ...
    template <typename T>
    double get_op_cost(std::unique_ptr<new_ops::Op> op,
                       const LabeledTensor<T> &labeled_tensor, bool is_update,
                       bool use_opmin = false) {
      std::unique_ptr<new_ops::Op> optree = op->clone();
      optree->set_attribute<new_ops::NeededLabelsAttribute>(labeled_tensor.labels());
      if (use_opmin) {

        new_ops::LTOp new_ltop{labeled_tensor.tensor(),
                               labeled_tensor.labels()};
        OpMin opmin{symbol_table_};
    ...
    template <typename T>
    size_t get_op_mem_cost(std::unique_ptr<new_ops::Op> op,
                           const LabeledTensor<T> &labeled_tensor,
                           bool use_opmin = false) {
