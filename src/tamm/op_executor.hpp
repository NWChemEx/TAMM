...
public:
  ...
  void print_tensor_execution_report(const Tensor<T>& tensor, bool use_opmin = true) {
    ...
    for(size_t update_idx = 0; update_idx < updates.size(); update_idx++) {
      ...
      for(size_t i = 0; i < canonicalized_ops.size(); i++) {
        ...
        std::cout << "Op cost : " << op_cost << std::endl;
  ...
  double get_op_cost(std::unique_ptr<new_ops::Op> op, const LabeledTensor<T>& labeled_tensor,
                     bool is_update = true, bool use_opmin = false) {
...
public:
  ...
  void execute(TensorVariant tensor_variant, bool use_opmin = false,
               ExecutionHW execute_on = ExecutionHW::CPU, bool profile = false) {
    std::visit(
      overloaded{[&](const auto& tensor) { execute(tensor, use_opmin, execute_on, profile); }},
  ...
  void execute(Tensor<T> tensor, bool use_opmin = false, ExecutionHW execute_on = ExecutionHW::CPU,
    ...
    if(!tensor.is_allocated()) { sch_.allocate(tensor); }
    ...
    for(size_t i = tensor.version(); i < updates.size(); i++) {
      ...
      for(size_t i = 0; i < canonicalized_ops.size(); i++) {
        ...
        // check if used tensors are allocated and have any updates
        {
          ...
          auto used_tensors = op->get_attribute<new_ops::UsedTensorInfoAttribute>().get();
  ...
  void print_op_cost(const Tensor<T>& tensor, bool use_opmin = false) {
    ...
    for(size_t i = 0; i < updates.size(); i++) {
      ...
      std::cout << "Total cost " << total_op_cost << std::endl;
    }
  }
