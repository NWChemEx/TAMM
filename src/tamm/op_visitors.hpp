...
public:
  ...
  void visit(MultOp& mop) override {
    ...
    mop.lhs().accept(*this);
    mop.rhs().accept(*this);
    const std::string& lhs_str   = mop.lhs().get_attribute<ToStringAttribute>().get();
    const std::string& rhs_str   = mop.rhs().get_attribute<ToStringAttribute>().get();
    std::string        coeff_str = mop.coeff().to_string();
    if(coeff_str == "1" || coeff_str == "1.0" || coeff_str == "1.00") { coeff_str = ""; }
    else { coeff_str += " * "; }
    mop.set_attribute<ToStringAttribute>("("s + coeff_str + lhs_str + " * " + rhs_str + ")");
  }
  ...
  void visit(AddOp& aop) override {
    ...
    aop.lhs().accept(*this);
    aop.rhs().accept(*this);
    const std::string& lhs_str = aop.lhs().get_attribute<ToStringAttribute>().get();
    const std::string& rhs_str = aop.rhs().get_attribute<ToStringAttribute>().get();
  ...
  void visit(LTOp& ltop) override {
    if(!ltop.has_attribute<ToStringAttribute>()) {
      ...
      if(use_ltop_) { result += "(tamm::new_ops::LTOp)"; }
      ...
      for(const auto& lbl: ltop.labels()) {
        if(symbol_table_.find(lbl.get_symbol_ptr()) == symbol_table_.end()) {
          if(lbl.label_str() == "") { result += std::to_string(lbl.label()) + ","; }
          else { result += "\"" + lbl.label_str() + "\","; }
        }
        else { result += symbol_table_[lbl.get_symbol_ptr()] + ","; }
      }
      result.pop_back();
      result += ")"s;
  ...
  ToStringVisitor(ToStringVisitor&& other) noexcept: ToStringVisitor{} { swap(*this, other); }
...
public:
  ...
  void visit(LTOp& ltop) override { ltop.set_attribute<AvailableLabelsAttribute>(ltop.labels()); }
  ...
  AvailableLabelsVisitor(AvailableLabelsVisitor&& other): AvailableLabelsVisitor{} {
    swap(*this, other);
  }
  ...
  friend void swap(AvailableLabelsVisitor& first, AvailableLabelsVisitor& second) noexcept {
    using std::swap;
    swap(static_cast<VisitorBase&>(first), static_cast<VisitorBase&>(second));
    // no-op
  }
...
public:
  ...
  NeededLabelsVisitor(NeededLabelsVisitor&& other): NeededLabelsVisitor{} { swap(*this, other); }
  ...
  friend void swap(NeededLabelsVisitor& first, NeededLabelsVisitor& second) noexcept {
    using std::swap;
    swap(static_cast<VisitorBase&>(first), static_cast<VisitorBase&>(second));
    // no-op
  }
...
private:
  void init(Op& op) {
    ...
    const auto& alabels = op.get_attribute<AvailableLabelsAttribute>().get();
  ...
  void apply_binop(Op& binop, Op& left_child, Op& right_child, bool is_mult_op = true) {
    const std::set<TiledIndexLabel>& nlabels = binop.get_attribute<NeededLabelsAttribute>().get();
    const std::set<TiledIndexLabel>& llabels =
      left_child.get_attribute<AvailableLabelsAttribute>().get();
    const std::set<TiledIndexLabel>& rlabels =
      right_child.get_attribute<AvailableLabelsAttribute>().get();
...
public:
  ...
  AllocLabelsVisitor(AllocLabelsVisitor&& other): AllocLabelsVisitor{} { swap(*this, other); }
  ...
  friend void swap(AllocLabelsVisitor& first, AllocLabelsVisitor& second) noexcept {
    using std::swap;
    swap(static_cast<VisitorBase&>(first), static_cast<VisitorBase&>(second));
    // no-op
  }
...
private:
  void init(Op& op) {
    ...
    std::set_intersection(alabels.begin(), alabels.end(), nlabels.begin(), nlabels.end(),
...
public:
  ...
  NameVisitor(const SymbolTable& symbol_table): symbol_table_{symbol_table} {}
...
public:
  ...
  void visit(LTOp& ltop) override { ltop.set_attribute<ElTypeAttribute>(ltop.tensor_type()); }
  ...
  ElTypeComputeVisitor(ElTypeComputeVisitor&& other): ElTypeComputeVisitor{} { swap(*this, other); }
  ...
  friend void swap(ElTypeComputeVisitor& first, ElTypeComputeVisitor& second) noexcept {
    using std::swap;
    swap(static_cast<VisitorBase&>(first), static_cast<VisitorBase&>(second));
  }
};
...
public:
  ...
  BinarizedPrintVisitor(const SymbolTable& symbol_table): symbol_table_{symbol_table} {}
  ...
  BinarizedPrintVisitor(BinarizedPrintVisitor&& other): BinarizedPrintVisitor{} {
    swap(*this, other);
  }
  ...
  friend void swap(BinarizedPrintVisitor& first, BinarizedPrintVisitor& second) noexcept {
    using std::swap;
    swap(static_cast<VisitorBase&>(first), static_cast<VisitorBase&>(second));
    swap(first.symbol_table_, second.symbol_table_);
  }
...
private:
  ...
  void init(Op& op) {
    ...
    if(coeff_str != "1" && coeff_str != "1.0") op_str += coeff_str + " * ";
    op_str += constructString(op);

    op.set_attribute<BinarizedStringAttribute>(std::vector<std::string>{op_str});
  }

  void apply_binop(Op& binop, const Op& left_child, const Op& right_child, std::string op_str) {
    ...
    std::string coeff_str = binop.coeff().to_string() == "1" ? ""
                                                             : binop.coeff().to_string() + " * ";
    ...
    else bin_str += " = " + coeff_str + lhs_str_vec.back();
  ...
  void apply_assign_op(Op& assignop, const Op& left_child, const Op& right_child,
    ...
    else assign_str += rhs_str_vec.back();
  ...
  std::string constructString(const Op& op) {
    ...
    const std::string&               name_str   = op.get_attribute<NameAttribute>().get();
    const std::set<TiledIndexLabel>& alloc_lbls = op.get_attribute<AllocLabelsAttribute>().get();
...
public:
  ...
  TensorInfoVisitor(const SymbolTable& symbol_table): symbol_table_{symbol_table} {}
  ...
  void visit(LTOp& ltop) override { init_lt(ltop); }
  ...
  TensorInfoVisitor(TensorInfoVisitor&& other): TensorInfoVisitor{} { swap(*this, other); }
  ...
  friend void swap(TensorInfoVisitor& first, TensorInfoVisitor& second) noexcept {
    using std::swap;
    swap(static_cast<VisitorBase&>(first), static_cast<VisitorBase&>(second));
    swap(first.symbol_table_, second.symbol_table_);
  }
...
private:
  ...
  void init(Op& op) {
    ...
    if(is_intermediate) { tensor = TensorVariant{t_eltype, t_ilv}; }
...
public:
  ...
  BinarizeOpsVisitor(const SymbolTable& symbol_table): symbol_table_{symbol_table} {}
  ...
  BinarizeOpsVisitor(BinarizeOpsVisitor&& other): BinarizeOpsVisitor{} { swap(*this, other); }
  ...
  friend void swap(BinarizeOpsVisitor& first, BinarizeOpsVisitor& second) noexcept {
    using std::swap;
    swap(static_cast<VisitorBase&>(first), static_cast<VisitorBase&>(second));
    swap(first.symbol_table_, second.symbol_table_);
...
struct OpStringGenerator {
  ...
  OpStringGenerator(const OpStringGenerator& osg): symbol_table_{osg.symbol_table_} {}
  OpStringGenerator operator=(OpStringGenerator osg) noexcept { swap(*this, osg); }
  OpStringGenerator(const SymbolTable& symbol_table): symbol_table_{symbol_table} {}
  OpStringGenerator(OpStringGenerator&& osg) noexcept: OpStringGenerator{} { swap(*this, osg); }

  std::string toString(const Op& op) {
    if(op.has_attribute<ToStringAttribute>()) return op.get_attribute<ToStringAttribute>().get();
...
public:
  ...
  ClearAttributesVisitor(ClearAttributesVisitor&& other) noexcept: ClearAttributesVisitor{} {
    swap(*this, other);
  }
  ...
  friend void swap(ClearAttributesVisitor& first, ClearAttributesVisitor& second) noexcept {
    using std::swap;
    swap(static_cast<VisitorBase&>(first), static_cast<VisitorBase&>(second));
  }
  ...
  void visit(LTOp& ltop) override { ltop.clear_attributes(); }
...
public:
  ...
  SummationLabelsVisitor(SummationLabelsVisitor&& other) noexcept: SummationLabelsVisitor{} {
    swap(*this, other);
  }
  ...
  friend void swap(SummationLabelsVisitor& first, SummationLabelsVisitor& second) noexcept {
    using std::swap;
    swap(static_cast<VisitorBase&>(first), static_cast<VisitorBase&>(second));
  }
...
private:
  ...
  void apply(Op& op) {
    ...
    const auto& available_labels = op.get_attribute<AvailableLabelsAttribute>().get();
    const auto& needed_labels    = op.get_attribute<NeededLabelsAttribute>().get();

    std::set<TiledIndexLabel> summation_labels;
    std::set_difference(available_labels.begin(), available_labels.end(), needed_labels.begin(),
                        needed_labels.end(),
...
public:
  ...
  SeparateSumOpsVisitor(SeparateSumOpsVisitor&& other) noexcept: SeparateSumOpsVisitor{} {
    swap(*this, other);
  }
  ...
  friend void swap(SeparateSumOpsVisitor& first, SeparateSumOpsVisitor& second) noexcept {
    using std::swap;
    swap(static_cast<VisitorBase&>(first), static_cast<VisitorBase&>(second));
    swap(first.sum_vectors_, second.sum_vectors_);
  }

  void visit(MultOp& mop) override { sum_vectors_.push_back(mop.clone()); }
  ...
  void visit(LTOp& ltop) override { sum_vectors_.push_back(ltop.clone()); }
...
public:
  ...
  UsedTensorInfoVisitor(const SymbolTable& symbol_table): symbol_table_{symbol_table} {}
  ...
  UsedTensorInfoVisitor(UsedTensorInfoVisitor&& other): UsedTensorInfoVisitor{} {
    swap(*this, other);
  }
  ...
  friend void swap(UsedTensorInfoVisitor& first, UsedTensorInfoVisitor& second) noexcept {
    using std::swap;
    swap(static_cast<VisitorBase&>(first), static_cast<VisitorBase&>(second));
    swap(first.symbol_table_, second.symbol_table_);
  }
...
public:
  ...
  void visit(MultOp& mop) override {
    ...
    for(const auto& il: all_labels) {
      ...
      else { this_cost *= il.tiled_index_space().index_space().max_num_indices(); }
    }
  ...
  void visit(LTOp& ltop) override { ltop.set_attribute<OpCostAttribute>(0.0); }
...
private:
  ...
  void init(Op& op) {
    if(!op.has_attribute<SummationLabelsAttribute>() || !op.has_attribute<AllocLabelsAttribute>()) {
...
public:
  ...
  ReplaceLTOpVisitor(ReplaceLTOpVisitor&& other) noexcept: ReplaceLTOpVisitor{} {
    swap(*this, other);
  }
  ...
  friend void swap(ReplaceLTOpVisitor& first, ReplaceLTOpVisitor& second) noexcept {
    using std::swap;
    swap(static_cast<VisitorBase&>(first), static_cast<VisitorBase&>(second));
  }
  ...
  void visit(LTOp& ltop) override {
    if(ltop.is_equal(target_op_)) { ltop = replace_op_; }
  }
...
public:
  ...
  DLPNOLabelsTopDownVisitor(const SymbolTable& symbol_table): symbol_table_{symbol_table} {}
  ...
  friend void swap(DLPNOLabelsTopDownVisitor& first, DLPNOLabelsTopDownVisitor& second) noexcept {
    using std::swap;
    swap(static_cast<VisitorBase&>(first), static_cast<VisitorBase&>(second));
  }
...
public:
  ...
  DLPNOLabelsVisitor(const SymbolTable& symbol_table): symbol_table_{symbol_table} {}

  DLPNOLabelsVisitor(DLPNOLabelsVisitor&& other) noexcept: DLPNOLabelsVisitor{} {
    swap(*this, other);
  }
  ...
  friend void swap(DLPNOLabelsVisitor& first, DLPNOLabelsVisitor& second) noexcept {
    using std::swap;
    swap(static_cast<VisitorBase&>(first), static_cast<VisitorBase&>(second));
  }
...
public:
  ...
  DLPNORewriteVisitor(const IndexLabelVec& lhs_pno_labels, const IndexLabelVec& lhs_lmo_pair_labels,
  ...
  DLPNORewriteVisitor(DLPNORewriteVisitor&& other) noexcept: DLPNORewriteVisitor{} {
    swap(*this, other);
  }
  ...
  friend void swap(DLPNORewriteVisitor& first, DLPNORewriteVisitor& second) noexcept {
    using std::swap;
    swap(static_cast<VisitorBase&>(first), static_cast<VisitorBase&>(second));
  ...
  void visit(MultOp& mop) override {
    ...
    mop.set_attribute<DLPNOStringAttribute>(
      mop.coeff().to_string() + "*" + mop.lhs().get_attribute<DLPNOStringAttribute>().get() + "*" +
  ...
  void visit(AddOp& aop) override {
    ...
    aop.lhs().accept(*this);
    aop.rhs().accept(*this);
    aop.set_attribute<DLPNOStringAttribute>(aop.lhs().get_attribute<DLPNOStringAttribute>().get() +
                                            "+\n" +
  ...
  void visit(LTOp& ltop) override {
    ...
    for(const auto& value: lhs_pno_str_) { lmo_pairs.insert(value.substr(value.rfind("_") + 1)); }
    ...
    if(name == "t1") {
      ...
      std::string transformed_str =
        tensor_cast_str_ + "dlpno_t1(\"" + a_ii + "\",\"" + ii + "\")" + stransform(a, ii);
      ...
      std::string transformed_str = tensor_cast_str_ + "dlpno_t2(\"" + a_ij + "\",\"" + b_ij +
                                    "\",\"" + ij + "\")" + stransform(a, ij) + stransform(b, ij);
      ...
      ltop.set_attribute<DLPNOStringAttribute>(tensor_cast_str_ +
                                               OpStringGenerator{symbol_table_}.toString(ltop));
    }
  }
...
private:
  ...
  bool has_external_label(const IndexLabelVec& tilv) {
    for(const auto& pno_lbl: lhs_pno_labels_) {
      for(const auto& lbl: tilv) {
        if(pno_lbl == lbl) { return true; }
      }
    }
    return false;
  }

  bool is_mo_v_label(const TiledIndexLabel& lbl) { return lbl.tiled_index_space() == mo_v_space_; }
  ...
  std::string constructVpair(const std::pair<TiledIndexLabel, TiledIndexLabel>& label_pair,
    ...
    if(is_mo_v_label(label_pair.first) && is_mo_v_label(label_pair.second)) {
      ...
      std::string te_vv =
        tensor_cast_str_ + "TEvv(\"" + first_rpao_str + "\",\"" + second_rpao_str + R"|(","K"))|";
      ...
      std::string first_label_str =
        find_lmo_pair(lmo_pairs, symbol_table_[label_pair.first.get_symbol_ptr()]);
      std::string second_label_str =
        find_lmo_pair(lmo_pairs, symbol_table_[label_pair.second.get_symbol_ptr()]);

      std::string te_oo =
        tensor_cast_str_ + "TEoo(\"" + first_label_str + "\",\"" + second_label_str + R"|(","K"))|";
      result += te_oo;
    }
    else {
      ...
      if(!is_mo_v_label(label_pair.first)) {
        ...
        oo_label_str = find_lmo_pair(lmo_pairs, o_label);
      ...
      if(o_lbl_loc != 0) { te_mix_str += "_1"; }

      std::string te_mix =
        tensor_cast_str_ + te_mix_str + "(\"" + oo_label_str + "\",\"" + rpao_str + R"|(","K"))|";
  ...
  std::string constructS(const std::string& ij, const std::string& lhs_ij, const std::string& a_ij,
                         const std::string& lhs_a_ij) {
    return tensor_cast_str_ + "S(\"" + ij + "\",\"" + lhs_ij + "\",\"" + a_ij + "\",\"" + lhs_a_ij +
           "\")";
  }

  std::string decomposedS(const std::string& ij, const std::string& lhs_ij, const std::string& a_ij,
                          const std::string& lhs_a_ij) {
    std::string a_str{a_ij[0]};
    std::string nutp_str = a_str + "nutp";
    std::string mutp_str = a_str + "mutp";
    std::string D1 =
      tensor_cast_str_ + "dinv(\"" + ij + "\",\"" + nutp_str + "\",\"" + a_ij + "\")";
    std::string D2 =
      tensor_cast_str_ + "d(\"" + lhs_ij + "\",\"" + mutp_str + "\",\"" + lhs_a_ij + "\")";
    std::string Stp = tensor_cast_str_ + "Stp(\"" + nutp_str + "\",\"" + mutp_str + "\")";

    return D1 + "*" + Stp + "*" + D2;
  }

  std::string find_lmo_pair(const std::set<std::string>& lmo_pairs, const std::string& mo_lbl) {
    for(const auto& pair: lmo_pairs) {
      if(pair.find(mo_lbl) != std::string::npos) { return pair; }
    }
...
public:
  ...
  DLPNOGuardedGenVisitor(const std::string& guard_name = ""): guard_name_{guard_name} {}
  DLPNOGuardedGenVisitor(const DLPNOGuardedGenVisitor&) = default;

  DLPNOGuardedGenVisitor(DLPNOGuardedGenVisitor&& other) noexcept: DLPNOGuardedGenVisitor{} {
    swap(*this, other);
  }
  ...
  friend void swap(DLPNOGuardedGenVisitor& first, DLPNOGuardedGenVisitor& second) noexcept {
    using std::swap;
    swap(static_cast<VisitorBase&>(first), static_cast<VisitorBase&>(second));
  ...
  void visit(AddOp& aop) override {
    aop.lhs().accept(*this);
    aop.rhs().accept(*this);
    std::string guarded_lhs = aop.lhs().get_attribute<DLPNOGuardedGenStringAttribute>("").get();
    std::string guarded_rhs = aop.rhs().get_attribute<DLPNOGuardedGenStringAttribute>("").get();
    aop.set_attribute<DLPNOGuardedGenStringAttribute>(guarded_lhs + guarded_rhs);
  }
  ...
  void visit(LTOp& ltop) override {
    ...
    if(dlpno == "0") { att += "// no-op for dlpno\n"; }
    else { att += dlpno_op_vector_name() + ".push_back((" + dlpno + ").clone());\n"; }
  ...
  std::string guard_declarations(int num_guards = -1) {
    std::string ret;
    if(num_guards == -1) { num_guards = guard_num_; }
  ...
  void reset_guard_num() { guard_num_ = 0; }
  ...
  void set_guard_name(const std::string& guard_name) { guard_name_ = guard_name; }
...
public:
  ...
  ReplaceLabelVisitor(ReplaceLabelVisitor&& other) noexcept: ReplaceLabelVisitor{} {
    swap(*this, other);
  }
  ...
  friend void swap(ReplaceLabelVisitor& first, ReplaceLabelVisitor& second) noexcept {
    using std::swap;
    swap(static_cast<VisitorBase&>(first), static_cast<VisitorBase&>(second));
  }
...
public:
  ...
  CanonicalizeVisitor(CanonicalizeVisitor&& other) noexcept: CanonicalizeVisitor{} {
    swap(*this, other);
  }
  ...
  friend void swap(CanonicalizeVisitor& first, CanonicalizeVisitor& second) noexcept {
    using std::swap;
    swap(static_cast<VisitorBase&>(first), static_cast<VisitorBase&>(second));
  }
...
private:
  ...
  void canonicalize_parfor_op(ParForOp& parforop) {
    ...
    for(const auto& lbl: labels) {
      if(lbl.tiled_index_space().num_tiles() == 1) { continue; }
      ...
      for(const auto& idx: lbl.tiled_index_space()) {
        ...
        canonicalized_ops_.push_back({std::move(internal_op), LabelPair{lbl, new_lbl}});
        no_slicing = false;
      }
    }
...
public:
  ...
  OpMemCostVisitor(const SymbolTable&                          symbol_table,
  ...
  OpMemCostVisitor(OpMemCostVisitor&& other): OpMemCostVisitor{} { swap(*this, other); }
...
private:
  ...
  size getMemSize(const Op& op) {
    ...
    if(info.is_intermediate_) { return info.tensor_.mem_size(); }
