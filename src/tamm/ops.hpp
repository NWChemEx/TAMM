...
constexpr auto LabeledTensor<T>::make_op(T1&& rhs, const bool is_assign, const int sub_v) {
  ...
  else if constexpr(is_complex_v<T> && (is_same_v<T1, LTT_int> || is_same_v<T1, LTT_float> ||
  ...
  // real=complex
    ...
    if constexpr(tuple_size_v<T1> == 2) {
      ...
      if constexpr((is_convertible_v<rhs0_t, T>) &&is_same_v<rhs1_t, LTT>)
        return AddOp<T, LTT, rhs1_t>{*this, static_cast<T>(sub_v) * static_cast<T>(get<0>(rhs)),
                        ...
                        (is_same_v<rhs1_t, LTT_int> || is_same_v<rhs1_t, LTT_float> ||
      ...
      //  LT = LT * LT
        ...
        return MultOp<T, LTT, LTT, LTT>{*this, static_cast<T>(sub_v), get<0>(rhs), get<1>(rhs),
                                        is_assign};
      // LHS is complex, rhs1,rhs2 are either complex/real
        ...
        return MultOp<T, LTT, rhs0_t, rhs1_t>{*this, sub_v, get<0>(rhs), get<1>(rhs), is_assign};
      // LHS is real, rhs1,rhs2 are either complex/real
      else if constexpr(!is_complex_v<T> &&
                        ((is_same_v<rhs0_t, LTT> &&
                          (is_same_v<rhs1_t, LTT_cfloat> || is_same_v<rhs1_t, LTT_cdouble>) ) ||
                         (is_same_v<rhs1_t, LTT> &&
                          (is_same_v<rhs0_t, LTT_cfloat> || is_same_v<rhs0_t, LTT_cdouble>) )))
        return MultOp<T, LTT, rhs0_t, rhs1_t>{*this, sub_v, get<0>(rhs), get<1>(rhs), is_assign};
    }
    ...
    // LT = alpha * LT * LT
      ...
      // LHS is complex, rhs1,rhs2 are either complex/real
        ...
        return MultOp<T, LTT, rhs1_t, rhs2_t>{*this, static_cast<rhs0_t>(sub_v * get<0>(rhs)),
                                              get<1>(rhs), get<2>(rhs), is_assign};
      // LHS is real, rhs1,rhs2 are either complex/real
      // alpha has to be real in this case
      else if constexpr(!is_complex_v<T> &&
                        ((is_same_v<rhs1_t, LTT> &&
                          (is_same_v<rhs2_t, LTT_cfloat> || is_same_v<rhs2_t, LTT_cdouble>) ) ||
                         (is_same_v<rhs2_t, LTT> &&
                          (is_same_v<rhs1_t, LTT_cfloat> || is_same_v<rhs1_t, LTT_cdouble>) )))
        return MultOp<T, LTT, rhs1_t, rhs2_t>{*this, static_cast<rhs0_t>(sub_v * get<0>(rhs)),
                                              get<1>(rhs), get<2>(rhs), is_assign};
