...
struct FlatPlan : public SetOpPlanBase<T, LabeledTensorT> {
  ...
  void apply(const SetOpT& setop, ExecutionContext& ec,
...
public:
    ...
    SetOp(LabeledTensorT lhs, T alpha, bool is_assign)
      ...
      if(!internal::is_slicing(lhs_) &&
         !internal::has_duplicates(lhs_.labels()) &&
         tensor.kind() != TensorBase::TensorKind::view ) {

          plan_     = Plan::flat;
          plan_obj_ = std::make_shared<internal::FlatPlan<T, LabeledTensorT>>();
          general_plan_obj_ =
            std::make_shared<internal::GeneralFlatPlan<T, LabeledTensorT>>();
      } else {

          plan_     = Plan::lhs;
          plan_obj_ = std::make_shared<internal::LHSPlan<T, LabeledTensorT>>();
          general_plan_obj_ =
            std::make_shared<internal::GeneralLHSPlan<T, LabeledTensorT>>();
      }
    ...
    TensorBase* accumulates(const ExecutionContext& ec) const {
      if (lhs_.tensor().pg() == ec.pg()) {
        ...
        general_plan_obj_->accumulates(*this);
      }
    }
...
template <typename T, typename LabeledTensorT>
void FlatPlan<T, LabeledTensorT>::apply(const SetOp<T, LabeledTensorT>& setop,
...
template <typename T, typename LabeledTensorT>
void LHSPlan<T, LabeledTensorT>::apply(
    const SetOp<T, LabeledTensorT>& setop, ExecutionContext& ec,
    ExecutionHW hw) {
    using LHS_ElType  = typename LabeledTensorT::element_type;
    auto lhs_lt       = setop.lhs();
    Scalar alpha      = setop.alpha();
    ...
    for (const auto& blockid : loop_nest) {
      ...
      if(!lhs_lt.tensor().is_non_zero(translated_blockid)) {
        continue;
      }

      auto [lhs_proc, lhs_offset] = ldist.locate(translated_blockid);
      if (tlb_valid && lhs_proc == me) {
        lambda(translated_blockid, lhs_offset);
      }
    }
}
...
template<typename T, typename LabeledTensorT>
void GeneralFlatPlan<T, LabeledTensorT>::apply(
  const SetOp<T, LabeledTensorT>& setop, ExecutionContext& ec, ExecutionHW hw) {
    using LHS_ElType = typename LabeledTensorT::element_type;
...
/// GeneralLHSPlan Method Implementations
template <typename T, typename LabeledTensorT>
void GeneralLHSPlan<T, LabeledTensorT>::apply(
    const SetOp<T, LabeledTensorT>& setop, ExecutionContext& ec,
    ExecutionHW hw) {
    using LHS_ElType = typename LabeledTensorT::element_type;
