...
public:
   ...
   */
  const TiledIndexSpaceVec& tiled_index_spaces() const { return impl_->tiled_index_spaces(); }
  ...
  static void allocate(const ExecutionContext* ec, SpinTensor<T>& tensor, Args&... rest) {
...
private:
   ...
   */
  template<typename... Args>
  static void alloc(const ExecutionContext* ec, SpinTensor<T>& tensor, Args&... rest) {
