...
inline void register_symbols_with_pos(std::map<void*, std::string>&   symbol_table,
...
void register_symbols_with_pos(std::map<void*, std::string>&   symbol_table,
  ...
  symbol_table[symbol.get_symbol_ptr()] = names[pos];
  register_symbols_with_pos(symbol_table, names, pos + 1, std::forward<Symbols>(symbols)...);
}
...
namespace tamm {
template<typename... Symbols>
void register_symbols(std::map<void*, std::string>& symbol_table, std::string names,
                      const SymbolInterface& symbol, Symbols&&... symbols) {
  ...
  while(ss.good()) {
    ...
    if(symbol_name[0] == ' ') { symbol_name = symbol_name.substr(1); }
