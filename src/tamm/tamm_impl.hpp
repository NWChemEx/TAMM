...
private:
  TAMM() {
    ...
    if(ngpus_ >= 1 || ngpus_ == 0) {
      std::string terminate_msg = "[NO-TALSH] Multiple/No GPUs detected per MPI rank, ngpus_(" + std::to_string(ngpus_) + ")!. Please consider using `tamm_gpu_bind.sh` script or job manager to bind...";
    ...
    mr_ = std::make_unique<pool_mr>(rmm::mr::get_current_device_resource());
