...
/**
 * @ingroup tensors
 * @brief Implementation class for the Sparse versin of the TensorBase class
 *
 * @tparam T Element type of Tensor
 */
template<typename T>
...
public:
  ...
  /**
   * @brief Construct a new SparseTensorImpl object using a vector of
   * TiledIndexSpace objects for each mode of the tensor
   *
   * @param [in] tis vector of TiledIndexSpace objects for each
   * mode used to construct the tensor
   */
  SparseTensorImpl(const TiledIndexSpaceVec& tis): TensorBase{tis} {
    has_spin_symmetry_    = false;
    has_spatial_symmetry_ = false;
    kind_                 = TensorBase::TensorKind::normal;
  }
   ...
   * the tensor
   */
  SparseTensorImpl(const std::vector<TiledIndexLabel>& lbls): TensorBase{lbls} {
    has_spin_symmetry_    = false;
    has_spatial_symmetry_ = false;
    kind_                 = TensorBase::TensorKind::normal;
  }
   ...
   */
  template<class... Ts>
  SparseTensorImpl(const TiledIndexSpace& tis, Ts... rest): TensorBase{tis, rest...} {
    num_modes_ = block_indices_.size();
    construct_dep_map();
    has_spin_symmetry_    = false;
    has_spatial_symmetry_ = false;
   ...
   * @param [in] spin_mask
   */
  SparseTensorImpl(TiledIndexSpaceVec t_spaces, SpinMask spin_mask): TensorBase(t_spaces) {
    EXPECTS(t_spaces.size() == spin_mask.size());
   ...
   * @param [in] spin_mask
   */
  SparseTensorImpl(IndexLabelVec t_labels, SpinMask spin_mask): TensorBase(t_labels) {
    EXPECTS(t_labels.size() == spin_mask.size());
    // for(const auto& tlbl : t_labels) {
    //     EXPECTS(tlbl.tiled_index_space().has_spin());
    // }
   ...
   * @param [in] spin_mask
   */
  SparseTensorImpl(IndexLabelVec t_labels, std::vector<size_t> spin_sizes): TensorBase(t_labels) {
    // EXPECTS(t_labels.size() == spin_mask.size());
    int spin_size_sum = std::accumulate(spin_sizes.begin(), spin_sizes.end(), 0);
    EXPECTS(t_labels.size() >= spin_size_sum);
    EXPECTS(spin_sizes.size() > 0);
  ...
  // Copy/Move Ctors and Assignment Operators
  SparseTensorImpl(SparseTensorImpl&&)                 = default;
  SparseTensorImpl(const SparseTensorImpl&)            = delete;
  SparseTensorImpl& operator=(SparseTensorImpl&&)      = default;
  SparseTensorImpl& operator=(const SparseTensorImpl&) = delete;
