...
public:
    ...
    TensorBase* base_ptr() const {
        return std::visit(
          overloaded{[&](const auto& tensor) { return tensor.base_ptr(); }},
          value_);
    }
    ...
    size_t mem_size() const {
      ...
      auto tis_list = std::visit(overloaded{[&](const auto &tensor) {
                                   return tensor.tiled_index_spaces();
                                 }},
                                 value_);
      for(const auto& tis : tis_list) {
        result *= tis.max_num_indices();
      }
      return result;
    }
    ...
    bool is_allocated() const {
      return std::visit(
          overloaded{[&](const auto &tensor) { return tensor.is_allocated(); }},
          value_);
    }

    bool has_ops() const {
      return std::visit(
          overloaded{[&](const auto &tensor) {
