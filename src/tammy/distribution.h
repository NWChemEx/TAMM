...
class Distribution_NW : public Distribution {
    ...
    std::pair<Proc, Offset> locate(const BlockDimVec& blockid) {
        ...
        proc -= 1;
        Offset offset =
          Offset{ioffset - proc_offsets_[proc.template value<size_t>()]
    ...
    Size buf_size(Proc proc) const {
        EXPECTS(proc >= Proc{0});
        EXPECTS(proc < nproc_);
        EXPECTS(proc_offsets_.size() >
                static_cast<uint64_t>(proc.template value<size_t>() + 1));
        return proc_offsets_[proc.template value<size_t>() + 1] -
               proc_offsets_[proc.template value<size_t>()];
    }
    ...
    for(auto& uitr = *tensor_structure_->unique_generator();
      ...
      if(tensor_structure_->nonzero(blockid)) {
        ...
        for(auto itr = pdt; itr != last; ++itr) {
            if(tensor_structure_->nonzero(*itr)) { length += 1; }
        }
        EXPECTS(length > 0);
        ...
        for(auto itr = pdt; itr != last; ++itr) {
            ...
            if(tensor_structure_->nonzero(blockid)) {
                ...
                hash_[length + addr] = offset;
                // if(GA_Nodeid() == 1) {
                //   //std::cerr<<"-----DISTRIBUTIO_NW. addr="<<addr<<"
                //   offset="<<offset<<"
                //   block_size="<<tensor_structure_->block_size(blockid)<<"\n";
                // }
                offset += tensor_structure_->block_size(blockid);
                addr += 1;
            }
        ...
        int64_t per_proc_size =
          std::max(int64_t{offset / nproc.template value<int64_t>()},
                   int64_t{1}); // max_2nd_arg);
        ...
        for(Proc i{0}; i < nproc; i++) {
            ...
            itr = std::lower_bound(
              itr, itr_last, (i + 1).template value<int64_t>() * per_proc_size);
            // if(GA_Nodeid() == 1) {
        ...
        EXPECTS(proc_offsets_.size() ==
                static_cast<uint64_t>(nproc.template value<size_t>()));
