...
static Range<Index> range(Index begin, Index end, Index step = 1) {
    Range<Index> ret;
    for(int i = begin; i < end; i += step) { ret.push_back(i); }
    return ret;
}

static Range<Index> range(Index end) { return range(Index{0}, end); }
...
class Attribute {
    ...
    std::vector<Range<Index>>::const_iterator begin() { NOT_IMPLEMENTED(); }
    std::vector<Range<Index>>::const_iterator end() { NOT_IMPLEMENTED(); }
...
class IndexSpaceImpl {
    ...
    virtual Index point(Index i,
                        const std::vector<Index>& indep_index = {}) const = 0;
...
class IndexSpace {
    ...
    IndexSpace(const IndexSpace& is, const RangeData& range,
               const SubspaceMap& named_subspaces = {}) :
      impl_{std::dynamic_pointer_cast<IndexSpaceImpl>(
    ...
    // std::initializer_list<IndexSpace>& spaces)
    //     :
    //     impl_{std::dynamic_pointer_cast<IndexSpaceImpl>(std::make_shared<AggregateSpaceImpl>(spaces))}
    //     {}
    ...
    //            const std::vector<std::string>& names = {},
    //            const std::map<std::string, std::vector<std::string>>&
    //            subspace_references = {}, const SubspaceMap& named_subspaces =
    //            {})
    //     :
    //     impl_{std::dynamic_pointer_cast<IndexSpaceImpl>(std::make_shared<AggregateSpaceImpl>(spaces,
    //     names, named_subspaces, subspace_references))} {}
    ...
    // Accessors
    Index point(Index i, const std::vector<Index>& indep_index = {});
    Index operator[](Index i) const;

    // IndexSpace accessors
    const IndexSpace& operator()(
      const std::vector<Index>& indep_index = {}) const;
...
class RangeIndexSpaceImpl : public IndexSpaceImpl {
    ...
    // Element Accesors
    Index point(Index i, const std::vector<Index>& indep_index = {}) const {
        return indices_[i];
    }
    ...
    // Iterators
    RangeIterator<Index> begin() const { return indices_.begin(); }
    RangeIterator<Index> end() const { return indices_.end(); }
...
class SubSpaceImpl : public IndexSpaceImpl {
    ...
    // TODO: optimization - constructing map<string, IndexSpace> can be delayed
    // until a specific subspace is requested
    SubSpaceImpl(const IndexSpace& is, const RangeData& range,
                 const SubspaceMap& named_ranges) :
      ref_space_{is},
    ...
    std::vector<Index> construct_indices(const IndexSpace& ref_space,
                                         const RangeData& range) {
        std::vector<Index> ret = {};
        for(const auto& i : ConstructIndexVector(range)) {
    ...
    SpinAttribute construct_spin(const IndexSpace& is, const RangeData& range) {
        // NOT_IMPLEMENTED();
...
class AggregateSpaceImpl : public IndexSpaceImpl {
    ...
    AggregateSpaceImpl(const std::initializer_list<IndexSpace>& spaces) :
      AggregateSpaceImpl(spaces, {}, {}, {}) {}
    ...
    std::vector<Index> construct_indices(
...
class DependentIndexSpaceImpl : public IndexSpaceImpl {
    ...
    // Iterators
    // TODO: Error on call
    RangeIterator<Index> begin() const { NOT_IMPLEMENTED(); }
    RangeIterator<Index> end() const { NOT_IMPLEMENTED(); }
...
Index IndexSpace::point(Index i, const std::vector<Index>& indep_index) {
...
// Iterators
RangeIterator<Index> IndexSpace::begin() const { return impl_->begin(); }
RangeIterator<Index> IndexSpace::end() const { return impl_->end(); }
