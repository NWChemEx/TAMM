#ifndef TAMMY_MEMORY_MANAGER_H_
#define TAMMY_MEMORY_MANAGER_H_
 ...
 *
 * @todo Check that offset+size for remote gets and puts is within the remote
 * size
 *
 ...
 * - Multiple memory regions can be allocated using the same memory manager and
 * share the same process group.
 * - Each memory manager implementation (sub-class) provides the implementation
 * of all operations on memory regions managed by it.
 * - The user accesses data in the memory regions using the memory region API's
 * routines. The memory region delegates to the implementation in the memory
 * manager.
 *
 * @note Memory regions use for their operations the memory manager that
 * allocated them. Thus, a memory manager should outlive all memory regions
 * created by it.
 */
class MemoryManager {
    public:
    ...
    protected:
    explicit MemoryManager(ProcGroup pg) : pg_{pg} {}
    ...
    /**
     * Access a pointer at an offset from local buffer associated with a memory
     * region.
     * @param mr Memory region being accessed
     * @param off Offset (in number of elements) from base of this rank's buffer
    ...
    void* access(MemoryRegion& mr, Offset off) {
        return const_cast<void*>(
          static_cast<const MemoryManager&>(*this).access(mr, off));
    }
    ...
    /**
     * Get data from a buffer associated with a memory region into a local
     * memory buffer
     ...
     */
    virtual void get(MemoryRegion& mr, Proc proc, Offset off, Size nelements,
                     void* buf) = 0;

    /**
     * Put data to a buffer associated with a memory region from a local memory
     * buffer
     * @param mr Memory region
     * @param proc Rank whose buffer is to be accessed
     * @param off Offset at which data is to be accessed
     ...
     * @pre buf[0..nelements] is valid (i.e., buffer is of sufficient size)
     */
    virtual void put(MemoryRegion& mr, Proc proc, Offset off, Size nelements,
                     const void* buf) = 0;

    /**
     * Add data to a buffer associated with a memory region from a local memory
     * buffer
     * @param mr Memory region
     * @param proc Rank whose buffer is to be accessed
     * @param off Offset at which data is to be accessed
     ...
     * @pre buf[0..nelements] is valid (i.e., buffer is of sufficient size)
     */
    virtual void add(MemoryRegion& mr, Proc proc, Offset off, Size nelements,
                     const void* buf) = 0;
 ...
 *
 * Memory regions allocate and manage one contiguous buffer per rank in the
 * corresponding memory manager's process group. Memory region contains
 * meta-data associated with an individual (collective allocation by a memory
 * manager. Memory regions are allocated using static calls in the memory
 * manager. Operations on the memory region delegate to the memory manager. This
 * base class provides some correctness checking by tracking the state of the
 * memory region as operations are performed on it.
 */
class MemoryRegion {
    public:
    /**
     ...
     *
     * @param nelements Number of elements to be allocated on this rank for this
     * memory manager.
     */
    ...
    /**
     * Has the memory region been attached  (using
     * MemoryManager::attach_coll())?
    ...
    virtual void put_impl(Proc proc, Offset off, Size nelements,
                          const void* buf)                                  = 0;
    virtual void add_impl(Proc proc, Offset off, Size nelements,
                          const void* buf)                                  = 0;
...
/**
 * @ingroup memory_management
 * @brief Implementation of the memory region operations that delegates to the
 * memory manager.
 *
...
class MemoryRegionImpl : public MemoryRegion {
    ...
    void put_impl(Proc proc, Offset off, Size nelements,
                  const void* buf) override {
        mgr_.put(*this, proc, off, nelements, buf);
    }

    void add_impl(Proc proc, Offset off, Size nelements,
                  const void* buf) override {
...
//#include "tammy/memory_manager_local.h"
//#include "tammy/memory_manager_ga.h"

#endif // TAMMY_MEMORY_MANAGER_H_
