...
class MemoryManagerGA : public MemoryManager {
     ...
     */
    MemoryRegion* alloc_coll(ElementType eltype,
                             Size local_nelements) override {
        ...
        MPI_Allreduce(&nels, &nelements_min, 1, MPI_LONG_LONG, MPI_MIN,
                      pg_.comm());
        MPI_Allreduce(&nels, &nelements_max, 1, MPI_LONG_LONG, MPI_MAX,
                      pg_.comm());
        if(nelements_min == nels && nelements_max == nels) {
            int64_t dim = nranks * nels, chunk = -1;
            pmr->ga_     = NGA_Create64(ga_eltype, 1, &dim,
                                    const_cast<char*>("array_name"), &chunk);
            pmr->map_[0] = 0;
            ...
            MPI_Allgather(&nels, 1, MPI_LONG_LONG, &pmr->map_[1], 1,
                          MPI_LONG_LONG, pg_.comm());
            pmr->map_[0] = 0; // @note this is not set by MPI_Exscan
            std::partial_sum(pmr->map_.begin(), pmr->map_.begin() + nranks,
                             pmr->map_.begin());
            std::string array_name{"array_name"};
            for(block = nranks; block > 0 && pmr->map_[block - 1] == dim;
                --block) {
                // no-op
            }
            int64_t* map_start = &pmr->map_[0];
            for(int i = 0; i < nranks && *(map_start + 1) == 0;
                ++i, ++map_start, --block) {
                // no-op
            }
            pmr->ga_ = NGA_Create_irreg64(ga_eltype, 1, &dim,
     ...
     */
    void fence(MemoryRegion& mr) { ARMCI_AllFence(); }

    protected:
    explicit MemoryManagerGA(ProcGroup pg) : MemoryManager{pg} {
        EXPECTS(pg.is_valid());
        ga_pg_ = create_ga_process_group_coll(pg);
    }

    ~MemoryManagerGA() { GA_Pgroup_destroy(ga_pg_); }
    ...
    static int create_ga_process_group_coll(const ProcGroup& pg) {
        ...
        MPI_Group_translate_ranks(group, nranks, ranks, group_world,
                                  ranks_world);
