...
using IndexLabel = std::vector<int>;
...
void four_index_transform(const AO& ao,
  ...
  Tensor<double> I0 = TensorImpl<double>::create(E | f1 + f2 | f3 + f4, perm_s4d);
  Tensor<double> I1 = TensorImpl<double>::create(E | p + f2 | f3 + f4, perm_n2d + perm_s2d);
  Tensor<double> I2 = TensorImpl<double>::create(E | p + q | f3 + f4, perm_a2d + perm_n2d);
  Tensor<double> I3 = TensorImpl<double>::create(E | p + q | r + f4, perm_s2d + perm_n2d);
...
compute_two_body_fock(Scheduler& sch,
  ...
  sch(
      G(a,b) = 0,
      G(a,b) += Integrals(a,c,b,d) * D(c,d),
      //    F(c,d) += (ab|cd) * D(a,b)
      G(c,d) += Integrals(a,c,b,d) * D(a,b),
      //    F(b,d) -= 1/4 * (ab|cd) * D(a,c)
      G(b,d) -= 0.25 * Integrals(a,c,b,d) * D(a,c),
      //    F(b,c) -= 1/4 * (ab|cd) * D(a,d)
      G(b,c) -= 0.25 * Integrals(a,c,b,d) * D(a,d),
      //    F(a,c) -= 1/4 * (ab|cd) * D(b,d)
      G(a,c) -= 0.25 * Integrals(a,c,b,d) * D(b,d),
      //    F(a,d) -= 1/4 * (ab|cd) * D(b,c)
      G(a,d) -= 0.25 * Integrals(a,c,b,d) * D(b,c),
