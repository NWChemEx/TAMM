...
// @to-do: This is just mock-up SubIndexSpace example. Should be implemented
// properly.
class SubAO_MSO : public SubIndexSpace {
    public:
    SubAO_MSO() = default; //@todo delete after debugging

    SubAO_MSO(MSO& mso, AO& ao) : SubIndexSpace{ao}, mso_{mso} {}
    ...
    //         std::back_inserter(offsets_)); for(BlockIndex i{0}; i<nov; i++) {
    ...
    int num_indep_indices() const override { return 1; }
    ...
    Iterator begin(
      ...
      const TensorVec<IndexSpace::Iterator>& bdv = {}) const override {
        ...
        switch(rv) {
            ...
            case range_v: off = no_; break;
            case range_e: off = no_ + nv_; break;
            default: UNREACHABLE();
        }
    ...
    Iterator end(RangeValue rv, const TensorVec<IndexSpace::Iterator>& bdv = {})
      const override {
        ...
        BlockCount off;
        switch(rv) {
            case range_n: off = no_ + nv_; break;
            case range_o: off = no_; break;
            case range_v: off = no_ + nv_; break;
            case range_e: off = no_ + nv_; break;
            default: UNREACHABLE();
        }
        using size_type = decltype(block_indices_)::size_type;
    ...
    IndexLabel O(Label label) const { return {OR(), label}; }

    IndexLabel V(Label label) const { return {VR(), label}; }
