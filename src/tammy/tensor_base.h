...
class TensorBase {
    ...
    TensorBase(const TensorVec<IndexRange>& dim_ranges) :
      dim_ranges_{dim_ranges},
      ipmask_{
        TensorVec<IndexPosition>(dim_ranges_.size(), IndexPosition::neither)},
    ...
    TensorBase(
      const std::tuple<TensorVec<IndexRange>, TensorVec<IndexRange>>& ranges,
      const PermGroup& perm_groups, Irrep irrep = Irrep{0},
      Spin spin_total = Spin{0}) :
      TensorBase{TensorBase::compute_index_range(ranges),
                 compute_ipmask(ranges), perm_groups, irrep, spin_total} {}

    TensorBase(
      const std::tuple<TensorVec<IndexRange>, TensorVec<IndexRange>>& ranges,
      Irrep irrep = Irrep{0}, Spin spin_total = Spin{0}) :
      TensorBase{
        TensorBase::compute_index_range(ranges), compute_ipmask(ranges),
        default_perm_group(compute_ipmask(ranges).size()), irrep, spin_total} {}

    TensorBase(const std::tuple<TensorVec<IndexRange>, TensorVec<IndexRange>,
                                TensorVec<IndexRange>>& ranges,
               Irrep irrep = Irrep{0}, Spin spin_total = Spin{0}) :
      TensorBase{compute_index_range(ranges), compute_ipmask(ranges),
                 default_perm_group(compute_ipmask(ranges).size()), irrep,
                 spin_total} {}

    bool spatial_nonzero(const BlockDimVec& bdv) const {
        if(!has_spatial_symmetry_) return true;
    ...
    bool spin_nonzero(const BlockDimVec& bdv) const {
        if(!has_spin_symmetry_) return true;
    ...
    const PermGroup& perm_group() const { return perm_groups_; }

    private:
    PermGroup default_perm_group(size_t rank) {
        TensorVec<size_t> indices;

        for(size_t i = 0; i < rank; i++) { indices.push_back(i); }
    ...
    void init_spin_check() {
        ...
        auto u =
          std::find(ipmask_.begin(), ipmask_.end(), IndexPosition::upper);
        auto l =
          std::find(ipmask_.begin(), ipmask_.end(), IndexPosition::lower);
    ...
    static TensorVec<IndexRange> compute_index_range(
      const std::tuple<TensorVec<IndexRange>, TensorVec<IndexRange>,
                       TensorVec<IndexRange>>& ranges) {
        TensorVec<IndexRange> ret;
        std::copy(std::get<0>(ranges).begin(), std::get<0>(ranges).end(),
                  std::back_inserter(ret));
        std::copy(std::get<1>(ranges).begin(), std::get<1>(ranges).end(),
                  std::back_inserter(ret));
        std::copy(std::get<2>(ranges).begin(), std::get<2>(ranges).end(),
                  std::back_inserter(ret));
        return ret;
    }
    ...
    static TensorVec<IndexRange> compute_index_range(
      const std::tuple<TensorVec<IndexRange>, TensorVec<IndexRange>>& ranges) {
        return compute_index_range(std::make_tuple(
          TensorVec<IndexRange>{}, std::get<0>(ranges), std::get<1>(ranges)));
    }

    static TensorVec<IndexPosition> compute_ipmask(
      const std::tuple<TensorVec<IndexRange>, TensorVec<IndexRange>,
                       TensorVec<IndexRange>>& ranges) {
        TensorVec<IndexPosition> ret;
        std::fill_n(std::back_inserter(ret), std::get<0>(ranges).size(),
                    IndexPosition::neither);
        std::fill_n(std::back_inserter(ret), std::get<1>(ranges).size(),
                    IndexPosition::upper);
        std::fill_n(std::back_inserter(ret), std::get<2>(ranges).size(),
                    IndexPosition::lower);
        return ret;
    }
...
class TensorImplBase {
    ...
    // virtual std::pair<Codelet*,Codelet*> get(const BlockDimVec& bdv,
    // Block<T>& block) const = 0;  virtual std::pair<Codelet*,Codelet*> put(const
    // BlockDimVec& bdv, Block<T>& block) = 0;  virtual
