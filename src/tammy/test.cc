...
int main()
{
  ...
  // auto T4 = Tensor<double>::create<TensorImpl<double>>(i1(k) + j1(l) | k | l);

  // Valid Tensor Creation
  auto V_T1 = Tensor<double>::create<TensorImpl<double>>(i1(j), j); // disabled for now
  ...
  auto V_T5 = Tensor<double>::create<TensorImpl<double>>(E | i1(k) + j1(l) | k + l);
  auto V_T6 = Tensor<double>::create<TensorImpl<double>>(i | j | k);
  auto V_T7 = Tensor<double>::create<TensorImpl<double>>(i + j| E | k1(j) + l);
