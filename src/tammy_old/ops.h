...
#include "tammy/labeled_block.h"
#include "tammy/labeled_tensor.h"
//#include "tammy/util.h"
//#include "tammy/fortran.h"
...
tensor_to_fortran_info(tammy::Tensor<T> &ttensor) {
  bool t_is_double = std::is_same<T, double>::value;
  EXPECTS(t_is_double);
  auto adst_nw = static_cast<const tammy::Distribution_NW *>(ttensor.distribution());
  ...
  auto amp_ga = static_cast<tammy::MemoryRegionGA&>(ttensor.memory_region());
...
AddOp<T, LabeledTensorType>::execute(const ProcGroup& ec_pg) {
  ...
  auto lambda = [&] (const BlockDimVec& cblockid) {
    //std::cout<<"---tammy assign. cblockid"<<cblockid<<std::endl;
    ...
    cbp() = 0;
    //std::cout<<"---tammy assign. ACTION ON cblockid"<<cblockid<<std::endl;
    ...
    for(; sit.has_more(); sit.next()) {
      ...
      //std::cout<<"---tammy assign. ACTION cur_cblockid"<<cur_cblockid<<std::endl;
