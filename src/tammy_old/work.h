...
#ifndef TAMMY_WORK_H_
#define TAMMY_WORK_H_

#include <algorithm>
#include "tammy/labeled_tensor.h"
#include "tammy/tensor.h"
#include "tammy/atomic_counter.h"
 ...
 */
template<typename Generator, typename Fn>
void
parallel_work_ga(ProcGroup pg, Generator gen, Fn fn) {
...
template<typename Generator, typename Fn>
void
parallel_work(const ProcGroup& pg, Generator gen, Fn fn) {
  // for(; first != last; ++first) {
  //   fn(*first);
  // }

  // parallel_work_omp(first, last, fn);
  parallel_work_ga(pg, gen, fn);
}
...
template<typename Generator, typename Fn>
void
parallel_work_ga(ProcGroup pg, Generator first, Generator last, Fn fn) {
  AtomicCounter *ac = new AtomicCounterGA(pg, 1);
  ac->allocate(0);
  int64_t next = ac->fetch_add(0, 1);
...
template<typename Generator, typename Fn>
void
seq_work(const ProcGroup& pg, Generator first, Generator last, Fn fn) {
...
template<typename Generator, typename Fn>
void
do_work(const ProcGroup& pg, Generator first, Generator last, Fn fn, ExecutionPolicy exec_policy) {
