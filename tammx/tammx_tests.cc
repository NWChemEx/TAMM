...
void test() {
  ...
  auto init_lambda = [](Block &block) {
    int n = 0;
    std::generate_n(reinterpret_cast<double *>(block.buf()), block.size(), [&]() { return n++; });
  };

  tensor_map(hT(), init_lambda);
  tensor_map(bT(), init_lambda);
  tensor_map(X_OO(), init_lambda);
  tensor_map(t2(), init_lambda);
...
//
  ...
  t1() += X_OO() * bT();
  assert_equal(t1, 20.0); //OK

  //A[], B[] -- zero-dimensional tensors
  //tensor_init(t1(), 0);
  //tensor_init(t2(), 2);
  t1() += 1.0 * t2();
  std::cerr << "T1 == ";
  tensor_print(t1,std::cerr);
