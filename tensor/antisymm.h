...
inline
tloop::tloop(const std::vector<size_t> &_indices, const int& _nloops)
  : indices(_indices), 
    nloops(_nloops), 
    first_time(true), 
    lb(0) {
...
inline 
antisymm::antisymm(const std::vector<size_t> &vtab,
...
inline bool 
antisymm::empty() const { return empty_; }

inline void 
antisymm::reset() { tl1 = tl0; }

inline int 
antisymm::sign() const { return 1; }
