...
void tamm_sort(double *sbuf, double *dbuf, const std::vector<size_t> &ids,
  ...
  if (ids.size() == 0) {
    ...
    Fint dimx = 1, permx = 4;
    ftce_sort_4(sbuf, dbuf, &dims[0], &dims[1], &dims[2], &dimx, &permx, &ip[0],
                &ip[1], &ip[2], &alpha);
  } else if (ids.size() == 4) {
...
void cdgemm(char transa, char transb, size_t m, size_t n, size_t k,
  ...
  fdgemm(&transa, &transb, &im, &in, &ik, &alpha, a, &ilda, b, &ildb, &beta, c,
         &ildc);
#else
...
void tamm_sortacc(double *sbuf, double *dbuf, const std::vector<size_t> &ids,
  ...
  if (ids.size() == 4) {
    ftce_sortacc_4(sbuf, dbuf, &dims[0], &dims[1], &dims[2], &dims[3], &ip[0],
                   &ip[1], &ip[2], &ip[3], &alpha);
  } else if (ids.size() == 6) {
    ftce_sortacc_6(sbuf, dbuf, &dims[0], &dims[1], &dims[2], &dims[3], &dims[4],
...
void tamm_restricted(int dim, int nupper, const std::vector<size_t> &value,
  ...
  if (dim == 1) {
    ...
    // assert(nupper==1);
    ftce_restricted_4(&dummy0, &ivalue[0], &ivalue[1], &ivalue[2], &dummy1,
...
static std::vector<size_t> invert_perm(std::vector<size_t> &perm) {
  ...
  for (size_t i = n; i >= 1; i--) {
    perm_out.push_back(n -
                       (std::find(perm.begin(), perm.end(), i) - perm.begin()));
  }
  return perm_out;
}

static size_t index_pair(int i, int j) { return i * (i - 1) / 2 + j; }

static bool is_spin_nonzero(const vector<size_t> &ids) {
  int lval = 0, rval = 0;
  Fint *int_mb = Variables::int_mb();
...
void cget_block_ind_i(gmem::Handle da, double *buf, size_t size, size_t offset,
  ...
  fget_block_ind_i(&ida, buf, &isize, &ikey, indexc, &is3, &is2, &is1, &is0);
#else
...

  {
    ...
    if (int_mb[k_spin + is[0]] == int_mb[k_spin + is[2]] &&
      ...
      // cout<<"perma perm="<<vperma[0]<<" "<<vperma[1]<<" "<<vperma[2]<<"
      // "<<vperma[3]<<endl;
...
void cget_hash_block_i(gmem::Handle da, double *buf, size_t size, size_t offset,
  ...
  Fint *int_mb = Variables::int_mb();
  Fint ikey = key;
  fget_hash_block_i(&ida, buf, &isize, &int_mb[offset], &ikey, &is3, &is2, &is1,
                    &is0);
#else
