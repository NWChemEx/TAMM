...
class Multiplication {
 ...
 public:
  ...
  Multiplication(Tensor* tC1, const std::vector<IndexName>& c_ids1, Tensor* tA1,
                 const std::vector<IndexName>& a_ids1, Tensor* tB1,
                 const std::vector<IndexName>& b_ids1, double coef);
...
inline std::vector<size_t> sort_ids(const std::vector<IndexName>& name,
                                    const std::vector<IndexName>& mem_pos_) {
  assert(name.size() == mem_pos_.size());
  std::vector<size_t> sort_ids_(name.size());
  for (int i = 0; i < name.size(); i++) {
    sort_ids_[i] =
        std::find(name.begin(), name.end(), mem_pos_[i]) - name.begin() + 1;
  }
  return sort_ids_;
}
...
inline std::vector<size_t> getMemPosVal(
    const std::vector<Index>& ids_, const std::vector<IndexName>& mem_pos_) {
...
inline Assignment::Assignment(Tensor* tC, Tensor* tA, double coef,
                              const std::vector<IndexName>& cids,
                              const std::vector<IndexName>& aids)
...
inline Tensor& Assignment::tC() { return *tC_; }

inline const Tensor& Assignment::tC() const { return *tC_; }

inline Tensor& Assignment::tA() { return *tA_; }

inline const Tensor& Assignment::tA() const { return *tA_; }

inline const std::vector<IndexName>& Assignment::cids() const { return cids_; }

inline const std::vector<IndexName>& Assignment::aids() const { return aids_; }

inline double Assignment::coef() { return coef_; }

inline IterGroup<triangular>& Assignment::out_itr() { return out_itr_; }

inline Multiplication::Multiplication() {}

inline Multiplication::~Multiplication() {}

inline Multiplication::Multiplication(Tensor* tC1, Tensor* tA1, Tensor* tB1,
                                      double coef)
...
inline Multiplication::Multiplication(
    Tensor* tC1, const std::vector<IndexName>& c_ids1, Tensor* tA1,
    const std::vector<IndexName>& a_ids1, Tensor* tB1,
    const std::vector<IndexName>& b_ids1, double coef)
    : tC_(tC1), tA_(tA1), tB_(tB1), coef_(coef) {
...
inline Tensor& Multiplication::tC() { return *tC_; }

inline const Tensor& Multiplication::tC() const { return *tC_; }

inline Tensor& Multiplication::tA() { return *tA_; }

inline const Tensor& Multiplication::tA() const { return *tA_; }

inline Tensor& Multiplication::tB() { return *tB_; }

inline const Tensor& Multiplication::tB() const { return *tB_; }

inline double Multiplication::coef() { return coef_; }

inline std::vector<IndexName>& Multiplication::sum_ids() { return sum_ids_; }

inline IterGroup<triangular>& Multiplication::out_itr() { return out_itr_; }

inline IterGroup<triangular>& Multiplication::sum_itr() { return sum_itr_; }

inline IterGroup<CopyIter>& Multiplication::cp_itr() { return cp_itr_; }
