...
void input_initialize(int num_ranges, RangeEntry *ranges, int num_indices,
                      IndexEntry *indices, int num_tensors,
                      TensorEntry *tensors, int num_operations,
                      Operation *ops) {
  ...
  for (int i = 0; i < num_tensors; i++) {
    ...
    int bug_irrep = 0;
    tensors[i].tensor =
        Tensor(tensors[i].ndim, tensors[i].nupper, bug_irrep, rts, bug_dist);
  }
