...
typedef enum { ANTI, TRIG, COPY, TRIG2 } IterType;
...
template <class T>
inline const bool& IterGroup<T>::empty() const {
  return empty_;
}

template <class T>
inline const int& IterGroup<T>::sign() const {
  return sign_;
}

template <class T>
inline const std::vector<size_t>& IterGroup<T>::v_range() const {
  return curr_r_;
}

template <class T>
inline const std::vector<size_t>& IterGroup<T>::v_offset() const {
  return curr_o_;
}

template <class T>
inline void IterGroup<T>::setType(IterType it) {
  type_ = it;
}
...
inline bool IterGroup<T>::next(std::vector<size_t>& vec) {
  ...
  if (type_ == TRIG2) {  // used in ccsd_t.cc
    ...
    curr_o_.clear();
    for (int i = 0; i < n; i++) {
      curr_r_.insert(curr_r_.end(), iters_[i].v_range().begin(),
                     iters_[i].v_range().end());
      curr_o_.insert(curr_o_.end(), iters_[i].v_offset().begin(),
                     iters_[i].v_offset().end());
    }
  }
