...
void pretty_print(const Equations &eqs) {
  ...
  for (int i = 0; i < eqs.op_entries.size(); i++) {
    ...
    const std::map<std::string, TensorEntry> &tes = eqs.tensor_entries;
    // int ta, tb, tc;
    switch (ope.optype) {
      case OpTypeAdd:
        os << "op" << ope.op_id << ": " << tes.at(ope.add.tc).name
           << index_to_string(eqs, tes.at(ope.add.tc).ndim, ope.add.tc_ids)
           << " += " << ope.add.alpha << " * " << tes.at(ope.add.ta).name
           << index_to_string(eqs, tes.at(ope.add.ta).ndim, ope.add.ta_ids)
           << ";" << endl;
        break;
      case OpTypeMult:
        os << "op" << ope.op_id << ": " << tes.at(ope.mult.tc).name
           << index_to_string(eqs, tes.at(ope.mult.tc).ndim, ope.mult.tc_ids)
           << " += " << ope.mult.alpha << " * " << tes.at(ope.mult.ta).name
           << index_to_string(eqs, tes.at(ope.mult.ta).ndim, ope.mult.ta_ids)
           << " * " << tes.at(ope.mult.tb).name
           << index_to_string(eqs, tes.at(ope.mult.tb).ndim, ope.mult.tb_ids)
           << ";" << endl;
        break;
      default:
        assert(0);
...
void canonicalize(Equations *eqs) {
  ...
  for (int i = 0; i < eqs->op_entries.size(); i++) {
    ...
    TensorEntry &cte = eqs->tensor_entries.at(ope.mult.tc);
    TensorEntry &ate = eqs->tensor_entries.at(ope.mult.ta);
    TensorEntry &bte = eqs->tensor_entries.at(ope.mult.tb);
...
void print_flatten(const Equations &eqs, int opid) {
    ...
    switch (ope.optype) {
      case OpTypeAdd:
        ...
        if (tensor_defs[ta] != -1) {
          ...
          TensorUse tu;
          tu.tensor_id = ta;
          int nadim = eqs.tensor_entries.at(ta).ndim;
      ...
      case OpTypeMult:
        ...
        if (tensor_defs[tb] != -1) {
          ...
          TensorUse tu;
          tu.tensor_id = tb;
          int nbdim = eqs.tensor_entries.at(tb).ndim;
  ...
  ope.mult.tc_ids;
  cout << tes.at(tc).name
        << index_to_string(eqs, tes.at(tc).ndim, tc_ids)
        << " += ";
