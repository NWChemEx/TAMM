...
void t_mult(double* a_c, Tensor& tC, Tensor& tA, Tensor& tB, const double coef,
            const vector<IndexName>& sum_ids, IterGroup<triangular>& sum_itr,
            IterGroup<CopyIter>& cp_itr, const vector<size_t>& tid,
            Multiplication& m) {
  ...
  while (ext_itr.next(ext_vec)) {
    if (tC.is_spatial_nonzero(ext_vec) && tC.is_spin_nonzero(ext_vec) &&
      ...
      while (sum_itr.next(sum_vec) || ONE_TIME) {
        ...
        int a_sign =
            sortByValueThenExtSymGroup(m.a_ids, a_name, a_svalue, a_svalue_r);
        ...
        tamm_sort(buf_a, buf_a_sort, a_svalue /*tA._value()*/,
                  a_sort_ids /*tA.sort_ids()*/, (double)a_sign /*tA.sign()*/);
        delete[] buf_a;

        double* buf_b = new double[dimb];
        double* buf_b_sort = new double[dimb];
        int b_sign =
            sortByValueThenExtSymGroup(m.b_ids, b_name, b_svalue, b_svalue_r);
