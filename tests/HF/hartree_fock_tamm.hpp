...
void compare_eigen_tamm_tensors(Tensor<TensorType> &tamm_tensor, Matrix &eigen_tensor,bool dprint=false){
  bool he=true;
  ...
  if(he) cout << "tamm - eigen tensors are equal\n";
  else cout << "tamm - eigen tensors are NOT equal\n";
...
//    std::cout << atoms[i].atomic_number << "  " << atoms[i].x<< "  " << atoms[i].y<< "  " << atoms[i].z << std::endl;
  ...
  auto one_body_kinetic_integral_lambda = [&](const IndexVector& blockid,
      ...
      // cout << "bufs, tbufs == " << buf.size() << "," << tbuf.size() << endl;
      // cout << "bufmat\n----------\n";
      // cout << buf_mat << endl;
...
//  Matrix F;
  ...
  do {
    ...
    //  if(iter <= 3 || simple_convergence) { cout << err_mat << endl; }
     ...
     cout << "-----------------------------------------------------------------"
             "--------\n";
     cout << "iter, ehf, ediff, rmsd = " << iter << "," << ehf << ", " << ediff
          << "," << rmsd << "\n";
