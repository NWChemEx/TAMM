...
int main(int argc, char* argv[]) {
  ...
  if(wrank == 0)
    std::cout << "Time (s) for MPI Barrier on MPI_COMM_WORLD with " << nproc
              << " ranks: " << gbarrier / ((double) nproc) << std::endl;
  ...
  if(my_rank == 0)
    std::cout << "Time (s) for MPI Barrier on GA_MPI_Comm with " << nproc
              << " ranks: " << gbarrier / ((double) nproc) << std::endl;
