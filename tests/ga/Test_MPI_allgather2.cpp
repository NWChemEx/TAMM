...
int main(int argc, char** argv) {
  ...
  tbeg           = MPI_Wtime();
  ierr           = MPI_Allgather(MPI_IN_PLACE, 1, MPI_LONG, hostid, 1, MPI_LONG, MPI_COMM_WORLD);
  ...
  for(i = 0; i < count; i++) {
    ...
    int imax;
    if(i < count - 1) { imax = start_index[i + 1]; }
    else { imax = nproc; }
    for(j = imin; j < imax; j++) {
      int k;
      for(k = j + 1; j < imax; j++) {
  ...
  int color = 0;
  if(me < nproc - 1) {
    if(hostid[me] != hostid[me + 1]) { color = 1; }
  }
  else { color = 1; }
  ...
  if(worker) {
    ...
    if(me == 0) { printf("Testing Allgather on working processors %d ranks\n", nproc); }
    for(i = 0; i < NLOOP; i++) {
      tbeg = MPI_Wtime();
      MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, send, sizeof(double), MPI_BYTE, my_comm);
      t_allgather_self += MPI_Wtime() - tbeg;
      tbeg = MPI_Wtime();
      MPI_Allgather(send, sizeof(double), MPI_BYTE, recv, sizeof(double), MPI_BYTE, my_comm);
  ...
  ierr = MPI_Barrier(MPI_COMM_WORLD);
  {
    ...
    if(me == 0) { printf("Testing Allgather on all processors %d ranks\n", nproc); }
    for(i = 0; i < NLOOP; i++) {
      tbeg = MPI_Wtime();
      MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, send, sizeof(double), MPI_BYTE,
                    MPI_COMM_WORLD);
      t_allgather_self += MPI_Wtime() - tbeg;
      tbeg = MPI_Wtime();
      MPI_Allgather(send, sizeof(double), MPI_BYTE, recv, sizeof(double), MPI_BYTE, MPI_COMM_WORLD);
      t_allgather_buf += MPI_Wtime() - tbeg;
    }
  ...
  if(me == 0) { printf("Total time in test program:               %16.6f\n", t_total); }
  ierr = MPI_Finalize();
}
