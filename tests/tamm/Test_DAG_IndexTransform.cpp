...
template<typename T>
OpList four_index_transform(const TiledIndexSpace& AO, const TiledIndexSpace& MSO, Tensor<T> tC,
                            Tensor<T> tV) {
...
template<typename T>
OpList two_index_transform(const TiledIndexSpace& AO, const TiledIndexSpace& MSO, Tensor<T> tC,
  ...
  Tensor<T> I0{MSO, AO};
  return {I0(p, f2) = tC(f1, p) * tF_ao(f1, f2), tF_mso(p, q) = tC(f1, q) * I0(f1, p)};
}
