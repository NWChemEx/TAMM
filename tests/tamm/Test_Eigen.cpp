...
public:
  EigenTensor(const std::array<long, ndim>& dims):
    Eigen::Tensor<double, ndim, Eigen::RowMajor>(dims) {}
};

template<typename T>
bool eigen_tensors_are_equal(EigenTensor<1>& e1, EigenTensor<1>& e2, double threshold = 1.0e-12) {
  bool ret  = true;
...
template<typename T>
bool eigen_tensors_are_equal(EigenTensor<2>& e1, EigenTensor<2>& e2, double threshold = 1.0e-12) {
  bool ret  = true;
  auto dims = e1.dimensions();
  for(auto i = 0; i < dims[0]; i++) {
...
template<typename T>
bool eigen_tensors_are_equal(EigenTensor<3>& e1, EigenTensor<3>& e2, double threshold = 1.0e-12) {
  bool ret  = true;
  auto dims = e1.dimensions();
  for(auto i = 0; i < dims[0]; i++) {
    for(auto j = 0; j < dims[1]; j++) {
...
template<typename T>
bool eigen_tensors_are_equal(EigenTensor<4>& e1, EigenTensor<4>& e2, double threshold = 1.0e-12) {
  bool ret  = true;
  auto dims = e1.dimensions();
  for(auto i = 0; i < dims[0]; i++) {
    for(auto j = 0; j < dims[1]; j++) {
...
template<int ndim>
inline PermEigen<ndim> eigen_perm_compute(const IndexLabelVec& from, const IndexLabelVec& to) {
...
template<int ndim>
void eigen_assign_dispatch(EigenTensorBase* tc, const IndexLabelVec& clabel, double alpha,
                           EigenTensorBase* ta, const IndexLabelVec& alabel) {
...
void eigen_assign(EigenTensorBase* tc, const IndexLabelVec& clabel, double alpha,
                  EigenTensorBase* ta, const IndexLabelVec& alabel) {
  EXPECTS(clabel.size() == alabel.size());
  if(clabel.size() == 0) {
    assert(0); //@todo implement
  }
  else if(clabel.size() == 1) { eigen_assign_dispatch<1>(tc, clabel, alpha, ta, alabel); }
  else if(clabel.size() == 2) { eigen_assign_dispatch<2>(tc, clabel, alpha, ta, alabel); }
  else if(clabel.size() == 3) { eigen_assign_dispatch<3>(tc, clabel, alpha, ta, alabel); }
  else if(clabel.size() == 4) { eigen_assign_dispatch<4>(tc, clabel, alpha, ta, alabel); }
  else {
    assert(0); //@todo implement
  }
}
...
void patch_copy(T* sbuf, Eigen::Tensor<T, ndim, Eigen::RowMajor>& etensor,
                const std::array<int, ndim>& block_dims, const std::array<int, ndim>& rel_offset) {
  assert(0);
}

template<typename T>
void patch_copy(T* sbuf, Eigen::Tensor<T, 1, Eigen::RowMajor>& etensor,
                const std::array<int, 1>& block_dims, const std::array<int, 1>& rel_offset) {
  int c = 0;
  for(auto i = rel_offset[0]; i < rel_offset[0] + block_dims[0]; i++, c++) { etensor(i) = sbuf[c]; }
}

template<typename T>
void patch_copy(T* sbuf, Eigen::Tensor<T, 2, Eigen::RowMajor>& etensor,
                const std::array<int, 2>& block_dims, const std::array<int, 2>& rel_offset) {
  int c = 0;
  for(auto i = rel_offset[0]; i < rel_offset[0] + block_dims[0]; i++) {
    for(auto j = rel_offset[1]; j < rel_offset[1] + block_dims[1]; j++, c++) {
      etensor(i, j) = sbuf[c];
    }
  }
}
...
template<typename T>
void patch_copy(T* sbuf, Eigen::Tensor<T, 3, Eigen::RowMajor>& etensor,
                const std::array<int, 3>& block_dims, const std::array<int, 3>& rel_offset) {
  int c = 0;
  for(auto i = rel_offset[0]; i < rel_offset[0] + block_dims[0]; i++) {
    for(auto j = rel_offset[1]; j < rel_offset[1] + block_dims[1]; j++) {
      for(auto k = rel_offset[2]; k < rel_offset[2] + block_dims[2]; k++, c++) {
...
template<typename T>
void patch_copy(T* sbuf, Eigen::Tensor<T, 4, Eigen::RowMajor>& etensor,
                const std::array<int, 4>& block_dims, const std::array<int, 4>& rel_offset) {
  int c = 0;
  for(auto i = rel_offset[0]; i < rel_offset[0] + block_dims[0]; i++) {
    for(auto j = rel_offset[1]; j < rel_offset[1] + block_dims[1]; j++) {
      for(auto k = rel_offset[2]; k < rel_offset[2] + block_dims[2]; k++) {
        for(auto l = rel_offset[3]; l < rel_offset[3] + block_dims[3]; l++, c++) {
...
template<typename T, int ndim>
EigenTensorBase* tamm_tensor_to_eigen_tensor_dispatch(Tensor<T>& tensor) {
  EXPECTS(tensor.num_modes() == ndim);

  std::array<long, ndim> dims;
  const auto&            tindices = tensor.tiled_index_spaces();
  for(int i = 0; i < ndim; i++) { dims[i] = tindices[i].index_space().num_indices(); }
...
template<typename T>
EigenTensorBase* tamm_tensor_to_eigen_tensor(Tensor<T>& tensor) {
  //   if(tensor.num_modes() == 0) {
  //     return tamm_tensor_to_eigen_tensor_dispatch<T,0>(tensor);
  //   } else
  if(tensor.num_modes() == 1) { return tamm_tensor_to_eigen_tensor_dispatch<T, 1>(tensor); }
  else if(tensor.num_modes() == 2) { return tamm_tensor_to_eigen_tensor_dispatch<T, 2>(tensor); }
  else if(tensor.num_modes() == 3) { return tamm_tensor_to_eigen_tensor_dispatch<T, 3>(tensor); }
  else if(tensor.num_modes() == 4) { return tamm_tensor_to_eigen_tensor_dispatch<T, 4>(tensor); }
  assert(0); //@todo implement
  return nullptr;
}

template<typename T>
void tamm_assign(ExecutionContext& ec, Tensor<T>& tc, const IndexLabelVec& clabel, double alpha,
                 Tensor<T>& ta, const IndexLabelVec& alabel) {
  auto& al = alabel;
  auto& cl = clabel;

  Scheduler{ec}((tc) (cl) += alpha * (ta) (al)).execute();
}

EigenTensorBase* eigen_assign(Tensor<double>& ttc, const IndexLabelVec& tclabel, double alpha,
                              Tensor<double>& tta, const IndexLabelVec& talabel) {
...
void tamm_tensor_fill(ExecutionContext& ec, LabeledTensorType ltensor) {
  ...
  for(auto it: tensor.loop_nest()) {
    ...
    tensor.get(it, buf);
    double n = std::rand() % 5;
    for(TAMM_SIZE i = 0; i < size; i++) { buf[i] = T{n + i}; }
    tensor.put(it, buf);
  }
}

bool test_eigen_assign_no_n(ExecutionContext& ec, double alpha, const IndexLabelVec& cupper_labels,
                            const IndexLabelVec& clower_labels, const IndexLabelVec& aupper_labels,
  ...
  Scheduler{ec}(ta() = 0)(tc1() = 0)(tc2() = 0).execute();
...
template<typename T>
void tamm_mult(ExecutionContext& ec, Tensor<T>& tc, const IndexLabelVec& clabel, double alpha,
               Tensor<T>& ta, const IndexLabelVec& alabel, Tensor<T>& tb,
               const IndexLabelVec& blabel) {
  auto& al = alabel;
  auto& bl = blabel;
  auto& cl = clabel;

  Scheduler{ec}((tc) () = 0.0)((tc) (cl) += alpha * (ta) (al) * (tb) (bl)).execute();
}

template<int ndim>
void eigen_mult_dispatch(EigenTensorBase* tc, const IndexLabelVec& clabel, double alpha,
                         EigenTensorBase* ta, const IndexLabelVec& alabel, EigenTensorBase* tb,
                         const IndexLabelVec& blabel) {
...
void eigen_mult(EigenTensorBase* tc, const IndexLabelVec& clabel, double alpha, EigenTensorBase* ta,
                const IndexLabelVec& alabel, EigenTensorBase* tb, const IndexLabelVec& blabel) {
  EXPECTS(clabel.size() == alabel.size());
  if(clabel.size() == 0) {
    assert(0); //@todo implement
  }
  else if(clabel.size() == 1) { eigen_mult_dispatch<1>(tc, clabel, alpha, ta, alabel, tb, blabel); }
  else if(clabel.size() == 2) { eigen_mult_dispatch<2>(tc, clabel, alpha, ta, alabel, tb, blabel); }
  else if(clabel.size() == 3) { eigen_mult_dispatch<3>(tc, clabel, alpha, ta, alabel, tb, blabel); }
  else if(clabel.size() == 4) { eigen_mult_dispatch<4>(tc, clabel, alpha, ta, alabel, tb, blabel); }
  else {
    assert(0); //@todo implement
  }
}

EigenTensorBase* eigen_mult(Tensor<double>& ttc, const IndexLabelVec& tclabel, double alpha,
                            Tensor<double>& tta, const IndexLabelVec& talabel, Tensor<double>& ttb,
...
bool test_eigen_mult_no_n(ExecutionContext& ec, double alpha, const IndexLabelVec& cupper_labels,
                          const IndexLabelVec& clower_labels, const IndexLabelVec& aupper_labels,
                          const IndexLabelVec& alower_labels, const IndexLabelVec& bupper_labels,
  ...
  Scheduler{ec}(ta() = 0)(tb() = 0)(tc1() = 0)(tc2() = 0).execute();
...
bool test_initval_no_n(ExecutionContext& ec, const IndexLabelVec& upper_labels,
  ...
  Scheduler{ec}(xta() = init_val)(xtc() = xta()).execute();
...
TEST_CASE("AssignTest - ZeroDim") {
  ...
  Tensor<double>::allocate(ec, xta, xtc);
  Scheduler{*ec}(xta() = init_val_a)(xtc() = init_val_c)(xtc() += alpha * xta()).execute();
...
TEST_CASE("MultTest - Dim_0_0_0") {
  ...
  Scheduler{*ec}(xta() = alpha1)(xtb() = alpha2)
    // fixme shud be =
    (xtc() += xta() * xtb())
      .execute();
  ...
  auto lambda = [&](Tensor<T>& t, const IndexVector& iv, std::vector<T>& buf) {
    for(auto& val: buf) status &= (std::abs(val - alpha1 * alpha2) < threshold);
  };

  Scheduler{*ec}.gop(xtc(), lambda).execute();
...
TEST_CASE("MultTest, Dim_o_0_o_up") {
  ...
  Tensor<double>::allocate(ec, xta, xtb, xtc);
  Scheduler{*ec}(xta() = alpha1)(xtb() = alpha2)(xtc() = 0)
    // fixme =
    (xtc() += xta() * xtb())
      .execute();
