...
std::tuple<TiledIndexSpace,TiledIndexSpace,TAMM_SIZE> setupTIS(TAMM_SIZE noa,TAMM_SIZE nva) {
    ...
    IndexSpace MO_IS{range(0, total_orbitals),
      ...
      if (tce_tile < 50 || tce_tile > 100)  {
        ...
        if(GA_Nodeid()==0) std::cout << std::endl << "Resetting tilesize to: " << tce_tile << std::endl;
      }
...
void test_io_3d(Scheduler& sch, TiledIndexSpace tis, TiledIndexSpace tis_i) {
  ...
  .allocate(t3_ooo,t3_oov,t3_ovv,t3_vvv)
  (t3_ook() = init_value)
  (t3_ovk() = init_value)
  (t3_vvk() = init_value)
