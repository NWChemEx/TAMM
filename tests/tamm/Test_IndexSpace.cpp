...
TEST_CASE("/* Test Case Description */") {
  ...
  IndexSpace is5{range(10), {{"occ", {range(0, 5)}}, {"virt", {range(5, 10)}}}};

  IndexSpace is6{is3, range(10)};
  IndexSpace is7{{is5("occ"), is5("virt")}};

  REQUIRE(is1 == is1);
  if(is1 == is1) { std::cout << "self-comparison" << std::endl; }

  REQUIRE(is1 == is2);
  if(is1 == is2) { std::cout << "is1 == is2" << std::endl; }

  REQUIRE(is1 != is3);
  if(is1 != is3) { std::cout << "is1 != is2" << std::endl; }

  REQUIRE(is1 != is4);
  if(is1 != is4) { std::cout << "is1 != is4" << std::endl; }

  REQUIRE(is3 != is4);
  if(is3 != is4) { std::cout << "is3 != is4" << std::endl; }

  REQUIRE(is1 == is5);
  if(is1 == is5) { std::cout << "is1 == is5" << std::endl; }

  REQUIRE(is1 == is6);
  if(is1 == is6) { std::cout << "is1 == is6" << std::endl; }

  REQUIRE(is1 == is7);
  if(is1 == is7) { std::cout << "is1 == is7" << std::endl; }

  REQUIRE(is2 == is5);
  if(is2 == is5) { std::cout << "is2 == is5" << std::endl; }

  REQUIRE(is2 == is6);
  if(is2 == is6) { std::cout << "is2 == is6" << std::endl; }

  REQUIRE(is2 == is7);
  if(is2 == is7) { std::cout << "is2 == is7" << std::endl; }

  REQUIRE(is5 == is5);
  if(is5 == is5) { std::cout << "is5 == is5" << std::endl; }

  REQUIRE(is5 == is6);
  if(is5 == is6) { std::cout << "is5 == is6" << std::endl; }

  REQUIRE(is5 == is7);
  if(is5 == is7) { std::cout << "is5 == is7" << std::endl; }

  REQUIRE(is6 == is7);
  if(is6 == is7) { std::cout << "is6 == is7" << std::endl; }
}
...
TEST_CASE("IndexSpace construction by range with named subspaces") {
  IndexSpace is{range(10), {{"occ", {range(2, 5)}}, {"virt", {range(4, 9)}}}};
  // check indices for full space
...
TEST_CASE(
  "IndexSpace construction with named subspaces by concatenation of other disjoint indexspaces") {
...
TEST_CASE("IndexSpace construction by subspacing with named subspaces") {
  ...
  IndexSpace sub_is{is, range(2, 8), {{"occ", {range(0, 3)}}, {"virt", {range(3, 6)}}}};
...
TEST_CASE("IndexSpace construction by aggregating with subnames") {
  ...
  IndexSpace agg_is{{temp_is, temp_is1},
                    {"occ", "virt"},
                    {{"local", {range(8, 13)}}},
                    {{"alpha", {"occ:alpha", "virt:alpha"}}, {"beta", {"occ:beta", "virt:beta"}}}};
...
TEST_CASE("IndexSpace construction using dependent IndexSpaces") {
  ...
  std::map<IndexVector, IndexSpace> dep_relation{
    {{0, 0}, is1}, {{0, 1}, is2}, {{1, 0}, temp_is2}, {{1, 1}, is2}};
...
TEST_CASE("IndexSpace construction for sub AO space dependent over ATOM index space") {
