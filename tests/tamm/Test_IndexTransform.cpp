...
void four_index_transform(ExecutionContext* ec, const TiledIndexSpace& AO,
                          const TiledIndexSpace& MSO, Tensor<T> tC, Tensor<T> tV) {
  ...
  Scheduler{ec} //(I0(f1, f2, f2, f4) = integral_function())
    .allocate(I0, I1, I2, I3)(I1(p, f2, f3, f4) = tC(f1, p) * I0(f1, f2, f3, f4))(
      I2(p, r, f3, f4) = tC(f2, r) * I1(p, f2, f3, f4))(
      I3(p, r, q, f4) = tC(f3, q) * I2(p, r, f3, f4))(tV(p, r, q, s) = tC(f4, s) * I3(p, r, q, f4))
...
void two_index_transform(ExecutionContext* ec, const TiledIndexSpace& AO,
                         const TiledIndexSpace& MSO, Tensor<T> tC, Tensor<T> tF_ao,
                         Tensor<T> tF_mso) {
  ...
  Tensor<T> I0{MSO, AO};
  Scheduler{ec}
    .allocate(I0)(I0(p, f2) = tC(f1, p) * tF_ao(f1, f2))(tF_mso(p, q) = tC(f1, q) * I0(p, f1))
    .deallocate(I0)
    .execute();
}
...
TEST_CASE("2/4-Index Transform") {
  ...
  IndexSpace MSO_IS{range(0, 200), {{"occ", {range(0, 100)}}, {"virt", {range(100, 200)}}}};
