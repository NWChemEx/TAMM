...
void check_value(LabeledTensor<T> lt, T val) {
  ...
  for(const auto& itval: loop_nest) {
    ...
    for(TAMM_SIZE i = 0; i < size; i++) {
      ...
      else { REQUIRE(std::abs(buf[i] - val) < 1.0e-10); }
    }
  }
}
...
void test_ops(const TiledIndexSpace& MO) {
  ...
  for(size_t i = 0; i < T1.block_size({1, 0, 1}); i++) EXPECTS(gbuf[i] == buf[i]);
...
TEST_CASE("Zero-dimensional ops") {
...

  {
    ...
    auto lambda2 = [](const Tensor<T>& t, const IndexVector& lhs_iv, std::vector<T>& lhs_buf,
                      const IndexVector rhs_iv[], std::vector<T> rhs_buf[]) {
      std::copy(rhs_buf[0].begin(), rhs_buf[0].end(), lhs_buf.begin());
    };
    auto lambda3 = [](const Tensor<T>& t, const IndexVector& lhs_iv, std::vector<T>& lhs_buf,
                      const IndexVector rhs_iv[], std::vector<T> rhs_buf[]) {
  ...
  {
    Tensor<T> T1{}, T2{};
    Scheduler{*ec}.allocate(T1, T2)(T2() = 42)(T1() = T2()).deallocate(T2).execute();
    check_value(T1, (T) 42.0);
    Tensor<T>::deallocate(T1);
  }
  ...
  {
    Tensor<T> T1{}, T2{};
    Scheduler{*ec}.allocate(T1, T2)(T1() = 3)(T2() = 42)(T1() += T2()).deallocate(T2).execute();
  ...
  {
    Tensor<T> T1{}, T2{};
    Scheduler{*ec}.allocate(T1, T2)(T1() = 42)(T2() = 3)(T1() -= T2()).deallocate(T2).execute();
  ...
  {
    Tensor<T> T1{}, T2{};
    Scheduler{*ec}.allocate(T1, T2)(T1() = 42)(T2() = 3)(T1() -= 4 * T2()).deallocate(T2).execute();
...

  {
    Tensor<T> T1{}, T2{}, T3{};
    Scheduler{*ec}
      .allocate(T1, T2, T3)(T1() = 0)(T2() = 3)(T3() = 5)(T1() += T2() * T3())
      .deallocate(T2, T3)
      .execute();
...
template<typename T>
bool test_mapop(ExecutionContext* ec, Tensor<T> T1, LabeledTensor<T> LT1, Tensor<T> T2,
                LabeledTensor<T> LT2, const std::vector<LabeledTensor<T>>& rest_lts = {}) {
  bool success = true;
  try {
    Tensor<T>::allocate(ec, T1);
    Tensor<T>::allocate(ec, T2);
    try {
      auto lambda = [](const Tensor<T>& t, const IndexVector& lhs_iv, std::vector<T>& lhs_buf,
                       const IndexVector rhs_iv[], std::vector<T> rhs_buf[]) {
        std::copy(rhs_buf[0].begin(), rhs_buf[0].end(), lhs_buf.begin());
      };
...
template<typename T>
bool test_addop(ExecutionContext* ec, Tensor<T> T1, Tensor<T> T2, LabeledTensor<T> LT1,
                LabeledTensor<T> LT2, std::vector<LabeledTensor<T>> rest_lts = {}) {
  bool success = true;
  ...
  try {
    success = true;
    Scheduler{*ec}(T1() = -1.0)(T1() = 4)(T2() = 42)(T1() += -3.1 * T2()).execute();
...
void test_setop_with_T(unsigned tilesize) {
  ...
  IndexSpace      IS{range(0, 10), {{"nr1", {range(0, 5)}}, {"nr2", {range(5, 10)}}}};
...

  {
    Tensor<T> T1{TIS, TIS, TIS};
    REQUIRE(test_setop(ec, T1, T1(l1, l2, l2),
                       {T1(l1, l1, l1), T1(l1, l1, l2), T1(l1, l2, l1), T1(l2, l1, l1),
                        T1(l2, l1, l2), T1(l2, l2, l1), T1(l2, l2, l2)}));
  }
  // 4-dimensional tests
  {
...

  {
    Tensor<T> T1{TIS, TIS, TIS, TIS};
    REQUIRE(
      test_setop(ec, T1, T1(l1, l2, l2, l1),
                 {T1(l1, l1, l1, l1), T1(l1, l1, l1, l2), T1(l1, l1, l2, l1), T1(l1, l1, l2, l2),
                  T1(l1, l2, l1, l1), T1(l1, l2, l1, l2), T1(l1, l2, l2, l2), T1(l2, l1, l1, l1),
                  T1(l2, l1, l1, l2), T1(l2, l1, l2, l1), T1(l2, l2, l1, l1), T1(l2, l2, l1, l2),
...
void test_mapop_with_T(unsigned tilesize) {
...

  {
    Tensor<T> T1{TIS, TIS, TIS, TIS}, T2{TIS, TIS, TIS, TIS};
    REQUIRE(
      test_mapop(ec, T1, T1(l1, l2, l2, l1), T2, T2(l1, l2, l2, l1),
                 {T1(l1, l1, l1, l1), T1(l1, l1, l1, l2), T1(l1, l1, l2, l1), T1(l1, l1, l2, l2),
                  T1(l1, l2, l1, l1), T1(l1, l2, l1, l2), T1(l1, l2, l2, l2), T1(l2, l1, l1, l1),
                  T1(l2, l1, l1, l2), T1(l2, l1, l2, l1), T1(l2, l2, l1, l1), T1(l2, l2, l1, l2),
                  T1(l2, l2, l2, l1), T1(l2, l2, l2, l2)}));
...
void test_addop_with_T(unsigned tilesize) {
  ...
  try {
    ...
    Tensor<T> T1{}, T2{}, T3{};
    Scheduler{*ec}
      .allocate(T1, T2, T3)(T1() = 0)(T2() = 8)(T3() = 4)(T1() += T2() * T3())
      .deallocate(T2, T3)
      .execute();
  ...
  try {
    ...
    Tensor<T> T1{}, T2{}, T3{};
    Scheduler{*ec}
      .allocate(T1, T2, T3)(T1() = 9)(T2() = 8)(T3() = 4)(T1() += 1.5 * T3() * T2())
      .deallocate(T2, T3)
      .execute();
  ...
  try {
    ...
    Tensor<T> T1{}, T2{}, T3{};
    Scheduler{*ec}
      .allocate(T1, T2, T3)(T1() = 9)(T2() = 8)(T3() = 4)(T3() += 1.5 * T1() * T2())
      .deallocate(T1, T2)
      .execute();
...
TEST_CASE("Three-dimensional mult ops part I") {
  ...
  try {
    ...
    Scheduler{*ec}
      .allocate(T1, T2, T3)(T1() = 2)(T2() = 3)(T3() = 4)(T1(i, j, k) += 1.7 * T2(i, j) * T3(k))
      .deallocate(T2, T3)
      .execute();
    check_value(T1, (T) (2 + 1.7 * 3 * 4));
    Tensor<T>::deallocate(T1);
  ...
  try {
    ...
    Scheduler{*ec}
      .allocate(T1, T2,
                T3)(T1() = 2)(T2() = 3)(T3() = 4)(T1(i, j, k) += 1.7 * T2(j, l, i) * T3(l, i, k))
      .deallocate(T2, T3)
      .execute();
...
TEST_CASE("Four-dimensional mult ops part I") {
  ...
  try {
    ...
    Scheduler{*ec}
      .allocate(T1, T2,
                T3)(T1() = 2)(T2() = 3)(T3() = 4)(T1(i, j, k, l) += 1.7 * T2(i, j) * T3(k, l))
      .deallocate(T2, T3)
      .execute();
    check_value(T1, (T) (2 + 1.7 * 3 * 4));
    Tensor<T>::deallocate(T1);
  ...
  try {
    ...
    Scheduler{*ec}
      .allocate(T1, T2, T3)(T1() = 2)(T2() = 3)(T3() = 4)(T1(i, j, k, l) +=
...
TEST_CASE("Two-dimensional ops part I") {
  ...
  try {
    failed = false;
    Tensor<T> T1{TIS, TIS}, T2{TIS, TIS};
    Scheduler{*ec}.allocate(T1, T2)(T1() = 4)(T2() = 42)(T1() += T2()).deallocate(T2).execute();
  ...
  try {
    failed = false;
    Tensor<T> T1{TIS, TIS}, T2{TIS, TIS};
    Scheduler{*ec}.allocate(T1, T2)(T1() = 4)(T2() = 42)(T1() += 3 * T2()).deallocate(T2).execute();
    check_value(T1, (T) 130.0);
    Tensor<T>::deallocate(T1);
  } catch(std::string& e) {
    std::cerr << "Caught exception: " << e << "\n";
  ...
  try {
    ...
    Tensor<T> T1{TIS, TIS}, T2{TIS, TIS}, T3{};
    Scheduler{*ec}
      .allocate(T1, T2, T3)(T1() = 4)(T2() = 42)(T3() = 5)(T1() += -3.1 * T2() * T3())
      .deallocate(T2, T3)
      .execute();
  ...
  try {
    ...
    Scheduler{*ec}
      .allocate(T1, T2, T3)(T1() = 4)(T2() = 42)(T3() = 5)(T1(i, j) += -3.1 * T2(i) * T3(j))
      .deallocate(T2, T3)
      .execute();
    check_value(T1, 4 - 3.1 * 42 * 5);
    Tensor<T>::deallocate(T1);
  ...
  try {
    ...
    std::tie(i, j) = TIS.labels<2>("all");
    Scheduler{*ec}
      .allocate(T1, T2, T3)(T1() = 4)(T2() = 42)(T3() = 5)(T1(i, j) += -3.1 * T2(i, j) * T3(i))
      .deallocate(T2, T3)
      .execute();
    check_value(T1, 4 - 3.1 * 42 * 5);
    Tensor<T>::deallocate(T1);
  ...
  try {
    ...
    std::tie(i, j) = TIS.labels<2>("all");
    Scheduler{*ec}
      .allocate(T1, T2, T3)(T1() = 4)(T2() = 42)(T3() = 5)(T1(i, j) += -3.1 * T2(j, i) * T3(i))
      .deallocate(T2, T3)
      .execute();
    check_value(T1, 4 - 3.1 * 42 * 5);
    Tensor<T>::deallocate(T1);
  ...
  try {
    ...
    std::tie(i, j) = TIS.labels<2>("all");
    Scheduler{*ec}
      .allocate(T1, T2, T3)(T1() = 4)(T2() = 42)(T3() = 5)(T1(i, j) += -3.1 * T2(i, j) * T3(j))
      .deallocate(T2, T3)
      .execute();
    check_value(T1, 4 - 3.1 * 42 * 5);
    Tensor<T>::deallocate(T1);
  ...
  try {
    ...
    std::tie(i, j) = TIS.labels<2>("all");
    Scheduler{*ec}
      .allocate(T1, T2, T3)(T1() = 4)(T2() = 42)(T3() = 5)(T1(i, j) += -3.1 * T3(j) * T2(j, i))
      .deallocate(T2, T3)
      .execute();
    check_value(T1, 4 - 3.1 * 42 * 5);
    Tensor<T>::deallocate(T1);
  ...
  try {
    ...
    Scheduler{*ec}
      .allocate(T1, T2, T3)(T1() = 4)(T2() = 42)(T3() = 5)(T1(i, j) += -3.1 * T2(i, j) * T3(i, j))
      .deallocate(T2, T3)
      .execute();
    check_value(T1, 4 - 3.1 * 42 * 5);
    Tensor<T>::deallocate(T1);
  ...
  try {
    ...
    std::tie(i, j, k) = TIS.labels<3>("all");
    Scheduler{*ec}
      .allocate(T1, T2, T3)(T1() = 4)(T2() = 42)(T3() = 5)(T1(i, j) += -3.1 * T2(i, k) * T3(j, k))
      .deallocate(T2, T3)
      .execute();
...
TEST_CASE("MultOp with RHS reduction") {
  ...
  IndexSpace      IS{range(0, 4), {{"nr1", {range(0, 2)}}, {"nr2", {range(2, 4)}}}};
  TiledIndexSpace TIS{IS, 1};
  ...
  try {
    ...
    std::tie(i, j, k) = TIS.labels<3>("all");
    Scheduler{*ec}
      .allocate(T1, T2, T3)(T1() = 4)(T2() = 3)(T3() = 5)(T1() += -3.1 * T2(i) * T3())
      .deallocate(T2, T3)
      .execute();
  ...
  try {
    ...
    std::tie(i, j, k) = TIS.labels<3>("all");
    Scheduler{*ec}
      .allocate(T1, T2, T3)(T1() = 4)(T2() = 3)(T3() = 5)(T1() += -3.1 * T2() * T3(j))
      .deallocate(T2, T3)
      .execute();
    check_value(T1, 4.0 - 3.1 * 4 * 3 * 5);
    Tensor<T>::deallocate(T1);
  ...
  try {
    ...
    std::tie(i, j, k) = TIS.labels<3>("all");
    Scheduler{*ec}
      .allocate(T1, T2, T3)(T1() = 4)(T2() = 3)(T3() = 5)(T1() += -3.2 * T2(i) * T3(j))
      .deallocate(T2, T3)
      .execute();
  ...
  try {
    ...
    IndexSpace      MO_IS{range(0, 14), {{"occ", {range(0, 10)}}, {"virt", {range(10, 14)}}}};
    ...
    sch(CV3D() = 42)(CV2D() = 42)(t1() = 2)(res() = 0)(res1() = 0)(sc1() = 0)(sc2() = 0)(
      tmp1() = 0)(tmp2() = 0)
      .execute();
    ...
    for(const IndexVector& blockid: CV3D.loop_nest()) {
      ...
      TAMM_SIZE c1 = 0;
      for(size_t i = block_offset[0]; i < block_offset[0] + block_dims[0]; i++) {
        for(size_t j = block_offset[1]; j < block_offset[1] + block_dims[1]; j++, c1++) {
