...
void test_unit_tiled_view_tensor(ExecutionContext& ec, size_t size, size_t tile_size) {
  ...
  sch
  (T_full("X", "Y", "Z") = A_full("X", "Y", "V") * B_full("V", "Z"))
  .execute();
  ...
  sch
  (T_unit_1("x", "Y", "Z") = A_unit_1("x", "Y", "V") * B_full("V", "Z"))
  .execute();
